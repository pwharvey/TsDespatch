...
Namespace TimberSmart.Security
    ...
    Public Class EventTrackerBase
        ...
        Public Overloads Shared Sub Process(ByVal page As ViewPage, ByVal request As PageRequest)
            Dim tracker As EventTracker = New EventTracker()
                ...
                If Not (field.ReadOnly) Then
                    ...
                    Else
                        If (IsCreatedByUserNamePattern(field.Name) OrElse IsModifiedByUserNamePattern(field.Name)) Then
                            field.TextMode = TextInputMode.Static
                            If (Not (String.IsNullOrEmpty(Email)) AndAlso String.IsNullOrEmpty(field.HyperlinkFormatString)) Then
        ...
        Protected Overloads Overridable Sub InternalProcess(ByVal args As ActionArgs, ByVal config As ControllerConfiguration)
            Dim hasCreatedByUserId As Boolean = false
            Dim hasCreatedByUserName As Boolean = false
            Dim hasCreatedOn As Boolean = false
            Dim hasModifiedByUserId As Boolean = false
            Dim hasModifiedByUserName As Boolean = false
            Dim hasModifiedOn As Boolean = false
            ...
            Dim fieldIterator As XPathNodeIterator = config.Select("/c:dataController/c:fields/c:field[not(@readOnly='true')]")
            Do While fieldIterator.MoveNext()
                ...
                If (args.CommandName = "Insert") Then
                    If (Not (hasCreatedByUserId) AndAlso IsCreatedByUserIdPattern(fieldName)) Then
                        hasCreatedByUserId = true
                        Dim v As FieldValue = New FieldValue(fieldName, UserId)
                        values.Add(v)
                    Else
                        If (Not (hasCreatedByUserName) AndAlso IsCreatedByUserNamePattern(fieldName)) Then
                            hasCreatedByUserName = true
                            Dim v As FieldValue = New FieldValue(fieldName, UserName)
                            values.Add(v)
                        Else
                            If (Not (hasCreatedOn) AndAlso IsCreatedOnPattern(fieldName)) Then
                                hasCreatedOn = true
                                Dim v As FieldValue = New FieldValue(fieldName, DateTime.Now)
                                values.Add(v)
                            End If
                        End If
                    End If
