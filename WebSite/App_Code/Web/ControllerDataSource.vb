...
Imports TimberSmart.Web.Design
...
Namespace TimberSmart.Web
    ...
    Public Class ControllerDataSource
        ...
        Protected Overrides Sub LoadViewState(ByVal savedState As Object)
            Dim pair As Pair = CType(savedState,Pair)
    ...
    Public Class FieldValueDictionary
        ...
        Public Sub Assign(ByVal values As IDictionary, ByVal assignToNewValues As Boolean)
            For Each fieldName As String in values.Keys
                ...
                End If
                Dim v As FieldValue = Me(fieldName)
    ...
    Public Class ControllerDataSourceView
        ...
        Protected Overrides Function ExecuteSelect(ByVal arguments As DataSourceSelectArguments) As IEnumerable
            Dim pageSize As Integer = Int32.MaxValue
            If (arguments.MaximumRows > 0) Then
                pageSize = arguments.MaximumRows
            End If
            Dim pageIndex As Integer = (arguments.StartRowIndex / pageSize)
            Dim request As PageRequest = Nothing
            If Not (String.IsNullOrEmpty(m_Owner.PageRequestParameterName)) Then
                Dim r As String = HttpContext.Current.Request.Params(m_Owner.PageRequestParameterName)
                If Not (String.IsNullOrEmpty(r)) Then
                    Dim serializer As System.Web.Script.Serialization.JavaScriptSerializer = New System.Web.Script.Serialization.JavaScriptSerializer()
                    request = serializer.Deserialize(Of PageRequest)(r)
            ...
            If (request Is Nothing) Then
                ...
                Dim filterValues As System.Collections.Specialized.IOrderedDictionary = FilterParameters.GetValues(HttpContext.Current, m_Owner)
                For Each p As Parameter in FilterParameters
                    Dim v As Object = filterValues(p.Name)
                    If (Not (v) Is Nothing) Then
                        Dim query As String = (p.Name + ":")
                        If ((p.DbType = DbType.Object) OrElse (p.DbType = DbType.String)) Then
                            For Each s As String in Convert.ToString(v).Split(Global.Microsoft.VisualBasic.ChrW(44), Global.Microsoft.VisualBasic.ChrW(59))
                                Dim q As String = Controller.ConvertSampleToQuery(s)
            ...
            Dim page As ViewPage = ControllerFactory.CreateDataController().GetPage(m_DataController, m_DataView, request)
        ...
        Protected Overrides Function ExecuteUpdate(ByVal keys As IDictionary, ByVal values As IDictionary, ByVal oldValues As IDictionary) As Integer
            Dim fieldValues As FieldValueDictionary = New FieldValueDictionary()
        ...
        Protected Function ExecuteAction(ByVal lastCommandName As String, ByVal commandName As String, ByVal fieldValues As FieldValueDictionary) As Integer
            ...
            Dim result As ActionResult = ControllerFactory.CreateDataController().Execute(DataController, DataView, args)
