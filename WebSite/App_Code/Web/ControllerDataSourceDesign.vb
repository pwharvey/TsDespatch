...
Namespace TimberSmart.Web.Design
    ...
    Public Class ControllerDataSourceDesigner
        ...
        Public Overrides Function GetView(ByVal viewName As String) As DesignerDataSourceView
            If Not (viewName.Equals(ControllerDataSourceView.DefaultViewName)) Then
                Return Nothing
            End If
            Dim webApp As IWebApplication = CType(Me.Component.Site.GetService(GetType(IWebApplication)),IWebApplication)
            If (webApp Is Nothing) Then
                Return Nothing
            End If
            Dim item As IProjectItem = webApp.GetProjectItemFromUrl("~/Controllers")
        ...
        Protected Overrides Sub PreFilterProperties(ByVal properties As IDictionary)
            ...
            Dim typeNameProp As PropertyDescriptor = CType(properties("DataController"),PropertyDescriptor)
    ...
    Public Class ControllerDataSourceDesignView
        ...
        Public Overrides ReadOnly Property Schema() As IDataSourceViewSchema
            Get
                ...
                Dim res As Stream = [GetType]().Assembly.GetManifestResourceStream(String.Format("TimberSmart.Controllers.{0}.xml", DataController))
                If (res Is Nothing) Then
                    res = [GetType]().Assembly.GetManifestResourceStream(String.Format("TimberSmart.{0}.xml", DataController))
                End If
                If (Not (res) Is Nothing) Then
                    document = New XPathDocument(res)
                Else
                    Dim dataControllerPath As String = Path.Combine(BasePath, (DataController + ".xml"))
        ...
        Public Overrides Function GetDesignTimeData(ByVal minimumRows As Integer, ByRef isSampleData As Boolean) As IEnumerable
            Dim fields() As IDataSourceFieldSchema = Schema.GetFields()
            Dim dt As DataTable = New DataTable(DataView)
            For Each field As IDataSourceFieldSchema in fields
            ...
            Do While (i < minimumRows)
                ...
                For Each field As IDataSourceFieldSchema in fields
                    Dim typeName As String = field.DataType.Name
                    Dim v As Object = i
    ...
    Public Class DataViewDesignSchema
        ...
        Function IDataSourceViewSchema_GetFields() As IDataSourceFieldSchema() Implements IDataSourceViewSchema.GetFields
            Dim fields As List(Of IDataSourceFieldSchema) = New List(Of IDataSourceFieldSchema)()
            If (Not (m_View) Is Nothing) Then
                Dim dataFieldIterator As XPathNodeIterator = m_View.Select(".//a:dataField", m_Nm)
                Do While dataFieldIterator.MoveNext()
                    fields.Add(New DataViewFieldSchema(dataFieldIterator.Current, m_Nm))
                Loop
                Dim systemFieldIterator As XPathNodeIterator = m_View.Select(String.Format("//a:field[not(@name=//a:view[@id='{0}']//a:dataField/@fieldName) and @isPrimaryKe"& _ 
    ...
    Public Class DataViewFieldSchema
        ...
        Public Sub New(ByVal fieldInfo As XPathNavigator, ByVal nm As XmlNamespaceManager)
            MyBase.New
            Dim field As XPathNavigator = fieldInfo
            If (fieldInfo.LocalName = "dataField") Then
                m_Name = fieldInfo.GetAttribute("fieldName", String.Empty)
                Dim aliasFieldName As String = fieldInfo.GetAttribute("aliasFieldName", String.Empty)
