    ...
    Partial Public Class DataControllerBase
        ...
        Private Sub ExecuteDataExport(ByVal args As ActionArgs, ByVal result As ActionResult)
            If Not (String.IsNullOrEmpty(args.CommandArgument)) Then
                Dim arguments() As String = args.CommandArgument.Split(Global.Microsoft.VisualBasic.ChrW(44))
                If (arguments.Length > 0) Then
                    Dim sameController As Boolean = (args.Controller = arguments(0))
                ...
                End If
            End If
            Dim request As PageRequest = New PageRequest(-1, -1, Nothing, Nothing)
        ...
        Private Sub ExportDataAsRowset(ByVal page As ViewPage, ByVal reader As DbDataReader, ByVal writer As StreamWriter)
            Dim s As String = "uuid:BDC6E3F0-6DA3-11d1-A2A3-00AA00C14882"
            Dim dt As String = "uuid:C2F41010-65B3-11d1-A29F-00AA00C14882"
            Dim rs As String = "urn:schemas-microsoft-com:rowset"
            Dim z As String = "#RowsetSchema"
            Dim settings As XmlWriterSettings = New XmlWriterSettings()
            settings.CloseOutput = false
            Dim output As XmlWriter = XmlWriter.Create(writer, settings)
            ...
            Dim fields As List(Of DataField) = New List(Of DataField)()
            For Each field As DataField in page.Fields
                If (Not ((field.Hidden OrElse field.OnDemand)) AndAlso Not (fields.Contains(field))) Then
                    Dim aliasField As DataField = field
            ...
            For Each field As DataField in fields
                ...
                Dim type As String = RowsetTypeMap(field.Type)
        ...
        Private Sub ExportDataAsRss(ByVal page As ViewPage, ByVal reader As DbDataReader, ByVal writer As StreamWriter)
            Dim appPath As String = Regex.Replace(HttpContext.Current.Request.Url.AbsoluteUri, "^(.+)Export.ashx.+$", "$1", RegexOptions.IgnoreCase)
            Dim settings As XmlWriterSettings = New XmlWriterSettings()
            settings.CloseOutput = false
            Dim output As XmlWriter = XmlWriter.Create(writer, settings)
            output.WriteStartDocument()
            ...
            Do While ((rowCount < MaximumRssItems) AndAlso reader.Read())
                ...
                Dim hasPubDate As Boolean = false
                ...
                Do While (i < page.Fields.Count)
                    ...
                    If Not (field.Hidden) Then
                        ...
                        If Not (hasTitle) Then
                            ...
                            Dim link As StringBuilder = New StringBuilder()
                            link.Append(m_Config.Evaluate("string(/c:dataController/@name)"))
                            For Each pkf As DataField in page.Fields
                                If pkf.IsPrimaryKey Then
                                    link.Append(String.Format("&{0}={1}", pkf.Name, reader(pkf.Name)))
                                End If
                            Next
                            Dim itemGuid As String = String.Format("{0}Details.aspx?l={1}", appPath, HttpUtility.UrlEncode(Convert.ToBase64String(Encoding.Default.GetBytes(link.ToString()))))
