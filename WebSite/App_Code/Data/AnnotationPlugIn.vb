...
Namespace TimberSmart.Data
    ...
    Public Class AnnotationPlugIn
        ...
        Public Shared ReadOnly Property AnnotationsPath() As String
            Get
                Dim p As String = WebConfigurationManager.AppSettings("AnnotationsPath")
        ...
        ReadOnly Property KeyFields() As String
            Get
                ...
                Dim iterator As XPathNodeIterator = Fields.Select("c:field[@isPrimaryKey='true']/@name", m_Config.Resolver)
        ...
        Public Overloads Shared Function GenerateDataRecordPath(ByVal controller As String, ByVal page As ViewPage, ByVal values() As FieldValue, ByVal rowIndex As Integer) As String
            ...
            Dim p As String = AnnotationPlugIn.AnnotationsPath
            If String.IsNullOrEmpty(controller) Then
                Dim handlerInfo As String = HttpContext.Current.Request("AnnotationPlugIn")
                Dim m As Match = Regex.Match(handlerInfo, "^((t|o|u)\|){0,1}\w+\|(\w+).+?\|(.+)?$")
                If m.Success Then
        ...
        Sub IPlugIn_PreProcessPageRequest(ByVal request As PageRequest, ByVal page As ViewPage) Implements IPlugIn.PreProcessPageRequest
            Dim view As XPathNavigator = m_Config.SelectSingleNode("//c:view[@id='{0}' and @type='Form']/c:categories", request.View)
            If ((Not (view) Is Nothing) AndAlso ((request.PageSize > 0) AndAlso (Not (request.Inserting) AndAlso (m_Config.SelectSingleNode("/c:dataController/c:fields/c:field[@name='_Annotation_NoteNew']") Is Nothing)))) Then
                ...
                Dim ns As String = ControllerConfiguration.Namespace
                Dim expressions As List(Of DynamicExpression) = New List(Of DynamicExpression)(m_Config.Expressions)
                ...
                Dim de As DynamicExpression = New DynamicExpression()
                ...
                Dim ade As DynamicExpression = New DynamicExpression()
        ...
        Sub IPlugIn_ProcessPageRequest(ByVal request As PageRequest, ByVal page As ViewPage) Implements IPlugIn.ProcessPageRequest
            ...
            If Not (m_RequireProcessing) Then
                ...
                Do While (i < page.Rows.Count)
                    Dim rowDir As String = AnnotationPlugIn.GenerateDataRecordPath(request.Controller, page, Nothing, i)
                ...
                Return
            End If
            Dim expressions As List(Of DynamicExpression) = New List(Of DynamicExpression)(page.Expressions)
            Dim de As DynamicExpression = New DynamicExpression()
            ...
            If Not (m_RetrieveAnnotations) Then
                Return
            End If
            Dim field As DataField = page.FindField("_Annotation_AttachmentNew")
            If (Not (field) Is Nothing) Then
                Dim fieldIndex As Integer = page.Fields.IndexOf(field)
                Dim newValue As String = String.Format("{0},{1}|{2}", request.Controller, field.Name, Regex.Replace(CType(page.Rows(0)(fieldIndex),String), "^\w+\|(.+)$", "$1"))
                If (field.Name = "_Annotation_AttachmentNew") Then
                    newValue = ("null|" + newValue)
                End If
                page.Rows(0)(fieldIndex) = newValue
            End If
            Dim p As String = AnnotationPlugIn.GenerateDataRecordPath(request.Controller, page, Nothing, 0)
            If Directory.Exists(p) Then
                Dim files() As String = Directory.GetFiles(p, "*.xml")
                Dim values As List(Of Object) = New List(Of Object)(page.Rows(0))
                Dim i As Integer = (files.Length - 1)
                Do While (i >= 0)
                    Dim filename As String = files(i)
                    Dim doc As XPathDocument = New XPathDocument(filename)
                    Dim nav As XPathNavigator = doc.CreateNavigator().SelectSingleNode("/*")
            ...
            Else
                ...
                Dim g As ActionGroup = New ActionGroup()
                page.ActionGroups.Add(g)
                g.Scope = "ActionBar"
                g.Flat = true
                Dim a As Action = New Action()
                g.Actions.Add(a)
        ...
        Private Function ReadNameAndEmail(ByVal nav As XPathNavigator) As String
            Dim userName As String = nav.GetAttribute("username", String.Empty)
            Dim email As String = nav.GetAttribute("email", String.Empty)
        ...
        Sub IPlugIn_ProcessArguments(ByVal args As ActionArgs, ByVal result As ActionResult, ByVal page As ViewPage) Implements IPlugIn.ProcessArguments
            If (m_Annotations.Count = 0) Then
                Return
            End If
            Dim p As String = AnnotationPlugIn.GenerateDataRecordPath(args.Controller, page, args.Values, 0)
            If Not (Directory.Exists(p)) Then
                Directory.CreateDirectory(p)
            End If
            For Each v As FieldValue in m_Annotations
                Dim m As Match = Regex.Match(v.Name, "^_Annotation_(Note)(New|\w+)$")
                If m.Success Then
                    If (m.Groups(1).Value = "Note") Then
                        Dim fileName As String = m.Groups(2).Value
                        ...
                        If Not (String.IsNullOrEmpty(Convert.ToString(v.NewValue))) Then
                            ...
                            Dim writer As XmlWriter = XmlWriter.Create(New FileStream(fileName, FileMode.Create), settings)
                            Try 
    ...
    Public Class AnnotationBlobHandler
        ...
        Public Overrides Function SaveFile(ByVal context As HttpContext, ByVal ba As BlobAdapter, ByVal keyValue As String) As Boolean
            ...
            Dim p As String = AnnotationPlugIn.GenerateDataRecordPath()
            If Not (Directory.Exists(p)) Then
                Directory.CreateDirectory(p)
            End If
            'u|OrderDetails,_Annotation_AttachmentNew|10248|11
            Dim m As Match = Regex.Match(Me.Value, "_Annotation_Attachment(\w+)\|")
            If m.Success Then
                Dim fileName As String = m.Groups(1).Value
                If (fileName = "New") Then
                    fileName = DateTime.Now.ToString("u")
                    fileName = Regex.Replace(fileName, "[\W]", String.Empty)
                ...
                If (file.ContentLength = 0) Then
                    For Each f As String in Directory.GetFiles(p, (Path.GetFileNameWithoutExtension(fileName) + "*.*"))
        ...
        Public Overrides Sub LoadFile(ByVal stream As Stream)
            ...
            Dim m As Match = Regex.Match(Me.Value, "_Annotation_Attachment(\w+)\|")
            Dim fileName As String = Path.Combine(p, (m.Groups(1).Value + ".xml"))
            Dim nav As XPathNavigator = New XPathDocument(fileName).CreateNavigator().SelectSingleNode("/*")
            ...
            Try 
                ...
                Dim bytesRead As Long = input.Read(buffer, 0, buffer.Length)
