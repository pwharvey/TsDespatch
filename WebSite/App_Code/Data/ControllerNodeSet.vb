...
Namespace TimberSmart.Data
    ...
    Public Class ControllerNodeSet
        ...
        Private Function DoReplaceVariable(ByVal m As Match) As String
            Dim o As Object = m_Args(m_ArgIndex)
        ...
        Private Function DoReplaceElementName(ByVal m As Match) As String
            ...
            Dim axis As String = m.Groups("Axis").Value
            Dim ns As String = m.Groups("Namespace").Value
        ...
        Public Function [Select](ByVal selector As String, ByVal ParamArray args() as System.[Object]) As ControllerNodeSet
            Dim m As Match = m_CreateElementRegex.Match(selector)
            If m.Success Then
        ...
        Private Function InternalSelect(ByVal add As Boolean, ByVal selector As String, ByVal ParamArray args() as System.[Object]) As ControllerNodeSet
            ...
            Dim list As List(Of XPathNavigator) = New List(Of XPathNavigator)()
            If add Then
                list.AddRange(m_Nodes)
            End If
            Dim rootNodes As List(Of XPathNavigator) = m_Nodes
            ...
            Else
                ...
                End If
            End If
            For Each root As XPathNavigator in rootNodes
                If (args.Length > 0) Then
                    m_Args = args
                    Do While (m_ArgIndex < args.Length)
                        Dim xpath As String = m_VariableRegex.Replace(selector, AddressOf DoReplaceVariable)
                        Dim iterator As XPathNodeIterator = root.Select(xpath, m_Resolver)
                        Do While iterator.MoveNext()
                            list.Add(iterator.Current.Clone())
                        Loop
                    Loop
                Else
                    Dim iterator As XPathNodeIterator = root.Select(selector, m_Resolver)
                    Do While iterator.MoveNext()
                        list.Add(iterator.Current.Clone())
                    Loop
                End If
        ...
        Public Function Delete() As ControllerNodeSet
            For Each node As XPathNavigator in m_Nodes
                node.DeleteSelf()
            Next
        ...
        Public Overloads Function Attr(ByVal name As String, ByVal value As Object) As ControllerNodeSet
            ...
            If TypeOf value Is Boolean Then
                s = s.ToLower()
            End If
            For Each nav As XPathNavigator in m_Nodes
                Dim attrNav As XPathNavigator = nav.SelectSingleNode(("@" + name))
        ...
        Public Overloads Function AppendTo(ByVal nodeSet As ControllerNodeSet) As ControllerNodeSet
            For Each node As XPathNavigator in Me.m_Nodes
                For Each parentNode As XPathNavigator in nodeSet.m_Nodes
        ...
        Public Function Arrange(ByVal selector As String, ByVal ParamArray sequence() as System.[String]) As ControllerNodeSet
            Dim i As Integer = (sequence.Length - 1)
            Do While (i >= 0)
                For Each node As XPathNavigator in m_Nodes
                    Dim seqNav As XPathNavigator = node.SelectSingleNode(selector, m_Resolver)
                    If ((Not (seqNav) Is Nothing) AndAlso (seqNav.Value = sequence(i))) Then
                        Dim sibling As XPathNavigator = node.Clone()
        ...
        Public Overloads Function Elem(ByVal name As String, ByVal value As Object) As ControllerNodeSet
            Dim s As String = Convert.ToString(value)
            Dim selector As String = ("c:" + name)
            For Each node As XPathNavigator in m_Nodes
                Dim elemNav As XPathNavigator = node.SelectSingleNode(selector, m_Resolver)
        ...
        Public Function SelectViews(ByVal ParamArray identifiers() as System.[String]) As ControllerNodeSet
            ...
            End If
            Dim searchByType As Boolean = false
            For Each s As String in New String() {"Grid", "Form", "DataSheet", "Chart", "Tree"}
        ...
        Public Function SelectDataFields(ByVal ParamArray fieldNames() as System.[String]) As ControllerNodeSet
            Dim list As List(Of XPathNavigator) = New List(Of XPathNavigator)()
            For Each node As XPathNavigator in m_Nodes
                Dim nodeSet As ControllerNodeSet = New ControllerNodeSet(Me, node)
        ...
        Public Function SelectActions(ByVal ParamArray commandNames() as System.[String]) As ControllerNodeSet
            ...
            End If
            Dim commandNameList As List(Of String) = New List(Of String)(commandNames)
        ...
        Public Overridable Function SetTag(ByVal name As String, ByVal value As String) As ControllerNodeSet
            Dim tagList As String = Attr("tag").Value()
        ...
        Public Overloads Function CreateActionGroup(ByVal id As String) As ControllerNodeSet
            Dim actionGroupNode As ControllerNodeSet = New ControllerNodeSet(m_Navigator, m_Resolver).Select("<actionGroup/>").AppendTo("/dataController/actions").Select("/dataController/actions/actionGroup[last()]")
        ...
        Public Overloads Function CreateAction(ByVal commandName As String, ByVal commandArgument As String, ByVal id As String) As ControllerNodeSet
            Dim actionNode As ControllerNodeSet = [Select]("<action/>").AppendTo(Me.Select("ancestor-or-self::actionGroup")).Select("action[last()]")
            If Not (String.IsNullOrEmpty(id)) Then
        ...
        Public Overloads Function CreateView(ByVal id As String, ByVal type As String, ByVal commandId As String) As ControllerNodeSet
            If String.IsNullOrEmpty(commandId) Then
                Dim commandIdNav As XPathNavigator = m_Navigator.SelectSingleNode("/c:dataController/c:commands/c:command/@id", m_Resolver)
        ...
        Public Overloads Function CreateCategory(ByVal id As String, ByVal headerText As String) As ControllerNodeSet
            For Each node As XPathNavigator in m_Nodes
                Dim parentNode As ControllerNodeSet = New ControllerNodeSet(Me, node)
                Dim categoriesNode As ControllerNodeSet = parentNode
        ...
        Public Overloads Function CreateDataField(ByVal fieldName As String, ByVal aliasFieldName As String) As ControllerNodeSet
            Dim existingFieldNode As ControllerNodeSet = SelectDataField(fieldName)
            If (existingFieldNode.Nodes.Count > 0) Then
                Return existingFieldNode
            End If
            For Each node As XPathNavigator in m_Nodes
                Dim parentNode As ControllerNodeSet = New ControllerNodeSet(Me, node)
                Dim dataFieldsNode As ControllerNodeSet = parentNode
                If Not ((node.Name = "dataFields")) Then
                    ...
                    End If
                End If
                Dim dataFieldNode As ControllerNodeSet = [Select]("<dataField/>").AppendTo(dataFieldsNode).Select("dataField[last()]").Attr("fieldName", fieldName)
        ...
        Public Function Move(ByVal target As ControllerNodeSet) As ControllerNodeSet
            If Not ((target.Nodes.Count = 1)) Then
                Return Me
            End If
            Dim targetNode As XPathNavigator = target.Nodes(0)
            For Each node As XPathNavigator in m_Nodes
                ...
                End If
                If Not (skip) Then
                    Dim newParent As XPathNavigator = targetNode
        ...
        Public Function Use() As ControllerNodeSet
            If (m_Nodes.Count > 0) Then
                ...
                Next
                Dim nodeName As String = m_Nodes(0).Name
                Dim parentNode As XPathNavigator = m_Nodes(0).SelectSingleNode("parent::*")
                parentNode.InnerXml = sb.ToString()
                Dim nodeSet As ControllerNodeSet = New ControllerNodeSet(Me, parentNode)
                Dim list As List(Of XPathNavigator) = New List(Of XPathNavigator)()
        ...
        Protected Function SelectFieldItemsNode() As ControllerNodeSet
            ...
            For Each node As XPathNavigator in m_Nodes
                Dim parentNode As ControllerNodeSet = New ControllerNodeSet(Me, node)
                Dim itemsNode As ControllerNodeSet = parentNode.Select("items")
        ...
        Public Function CreateItem(ByVal value As Object, ByVal text As String) As ControllerNodeSet
            Dim itemsNode As ControllerNodeSet = [Select]("items")
            If (itemsNode.Nodes.Count = 0) Then
        ...
        Protected Function CreateExpression(ByVal rootElement As String, ByVal ParamArray attributes() as System.[String]) As ControllerNodeSet
            For Each node As XPathNavigator in m_Nodes
                Dim nodeSet As ControllerNodeSet = New ControllerNodeSet(Me, node)
                Dim rootNode As ControllerNodeSet = nodeSet.Select(rootElement)
                If (rootNode.Nodes.Count = 0) Then
                    [Select](String.Format("<{0}/>", rootElement)).AppendTo(nodeSet)
                    rootNode = nodeSet.Select(rootElement)
                End If
                Dim expressionNode As ControllerNodeSet = nodeSet.Select("expression[1]")
        ...
        Function SelectBusinessRules(ByVal filter As String) As ControllerNodeSet
            Dim selector As String = "/dataController/businessRules/rule"
        ...
        Public Overloads Function CreateBusinessRule(ByVal type As String, ByVal phase As String, ByVal commandName As String, ByVal commandArgument As String, ByVal view As String, ByVal script As String, ByVal name As String) As ControllerNodeSet
            Dim businessRulesNode As ControllerNodeSet = [Select]("/dataController/businessRules")
            ...
            End If
            Dim ruleNode As ControllerNodeSet = [Select]("<rule/>").AppendTo(businessRulesNode).Select("rule[last()]")
        ...
        Public Overloads Function CreateField(ByVal name As String, ByVal type As String, ByVal formula As String) As ControllerNodeSet
            Dim fieldsNode As ControllerNodeSet = [Select]("/dataController/fields")
            ...
            If String.IsNullOrEmpty(type) Then
                type = "String"
            End If
            Dim fieldNode As ControllerNodeSet = [Select]("<field/>").AppendTo(fieldsNode).Select("field[last()]").Attr("name", name).Attr("type", type)
            If (type = "String") Then
        ...
        Public Overloads Function StatusBar(ByVal formula As String, ByVal type As String, ByVal statusMap As String) As ControllerNodeSet
            ...
            End If
            Dim statusBarNode As ControllerNodeSet = [Select]("/dataController/statusBar")
