    ...
    Public Class Localizer
        ...
        Public Overloads Shared Function Replace(ByVal baseName As String, ByVal objectName As String, ByVal text As String) As String
            If Not (TokenRegex.IsMatch(text)) Then
                Return text
            End If
            Dim l As Localizer = New Localizer(baseName, objectName, text)
            Return l.Replace()
        End Function
        ...
        Public Overloads Overridable Function Replace() As String
            ...
            If (m_BaseName = "Pages.") Then
                m_ScriptMode = true
                Dim stateInput As String = StateRegexReplace
                If (HttpContext.Current.Request.Browser.Browser = "IE") Then
                    stateInput = StateRegexReplaceIE
                End If
                Dim output As String = StateRegex.Replace(ScriptRegex.Replace(m_Text.Trim(), AddressOf DoReplaceScript), stateInput)
                m_ScriptMode = false
        ...
        Public Shared Function CreateDictionaryStream(ByVal culture As String, ByVal baseName As String, ByVal objectName As String, ByRef files() As String) As Stream
            files = Nothing
            Dim fileName As String = String.Format("{0}.txt", objectName)
            Dim t As Type = GetType(Controller)
            Dim result As Stream = t.Assembly.GetManifestResourceStream(CultureManager.ResolveEmbeddedResourceName(String.Format("TimberSmart.{0}{1}", baseName, fileName), culture))
            If (result Is Nothing) Then
                result = t.Assembly.GetManifestResourceStream(CultureManager.ResolveEmbeddedResourceName(String.Format("TimberSmart.{0}", fileName), culture))
            End If
            If (result Is Nothing) Then
                fileName = String.Format("{0}.{1}.txt", objectName, culture)
                Dim objectPath As String = Path.Combine(Path.Combine(HttpRuntime.AppDomainAppPath, baseName), fileName)
                ...
                Else
                    If (String.IsNullOrEmpty(baseName) AndAlso (objectName = "CombinedSharedResources")) Then
                        Dim dependencies As List(Of String) = New List(Of String)()
                        result = New MemoryStream()
                        Dim root As String = HttpContext.Current.Server.MapPath("~/")
                        Dim list() As String = Nothing
                        'try loading "Resources.CULTURE-NAME.txt" files
                        Dim rs As Stream = CreateDictionaryStream(culture, String.Empty, "Resources", list)
                        ...
                        Dim controlsPath As String = Path.Combine(root, "Controls")
                        If Directory.Exists(controlsPath) Then
                            For Each f As String in Directory.GetFiles(controlsPath, "*.ascx")
        ...
        Public Shared Function CreateDictionary(ByVal baseName As String, ByVal objectName As String) As LocalizationDictionary
            ...
            Dim fileName As String = String.Format("TimberSmart.{0}.{1}.txt", objectName, culture)
            Dim dictionary As LocalizationDictionary = CType(HttpRuntime.Cache(fileName),LocalizationDictionary)
            If (dictionary Is Nothing) Then
                dictionary = New LocalizationDictionary()
                Dim files() As String = Nothing
                Dim s As Stream = CreateDictionaryStream(culture, baseName, objectName, files)
        ...
        Private Shared Sub PopulateDictionary(ByVal dictionary As LocalizationDictionary, ByVal text As String)
            Dim m As Match = TokenRegex.Match(text)
            Do While m.Success
                Dim token As String = m.Groups(1).Value
                If (token = m.Groups(3).Value) Then
