...
Namespace TimberSmart.Data
    ...
    Public Class FilterFunctions
        ...
        Public Shared Function Replace(ByVal command As DbCommand, ByVal expressions As SelectClauseDictionary, ByVal filter As String) As String
            Dim functions As FilterFunctions = New FilterFunctions(command, expressions, filter)
            Return functions.ToString()
        End Function
        
        Public Overrides Function ToString() As String
            Dim filter As String = Regex.Replace(Me.m_Filter, "\$((?'Name'\w+)\s*\((?'Arguments'[\s\S]*?)\)\s*)", AddressOf DoReplaceFunctions)
            Return filter
        End Function
    ...
    Public Class FilterFunctionBase
        ...
        Protected Function CreateParameter(ByVal command As DbCommand) As DbParameter
            ...
            Dim marker As String = SqlStatement.ConvertTypeToParameterMarker(p.GetType())
        ...
        Public Function FirstArgument(ByVal arguments As String) As String
            Dim m As Match = Regex.Match(arguments, "^\s*(\w+)\s*(,|\$comma\$)?")
            Return m.Groups(1).Value
        End Function
        
        Protected Function ExtractArgument(ByVal arguments As String, ByVal index As Integer) As String
            Dim m As Match = Regex.Match(arguments, "^\s*(\w+)\s*(,|\$comma\$)\s*([\s\S]*?)\s*$")
            Dim s As String = m.Groups(3).Value
    ...
    Public Class UserNameFilterFunction
        ...
        Public Overrides Function ExpandWith(ByVal command As DbCommand, ByVal expressions As SelectClauseDictionary, ByVal arguments As String) As String
            Dim p As DbParameter = CreateParameter(command)
    ...
    Public Class BetweenFilterFunction
        ...
        Public Overrides Function ExpandWith(ByVal command As DbCommand, ByVal expressions As SelectClauseDictionary, ByVal arguments As String) As String
            ...
            Dim p2 As DbParameter = CreateParameter(command)
    ...
    Public Class InFilterFunction
        ...
        Public Overrides Function ExpandWith(ByVal command As DbCommand, ByVal expressions As SelectClauseDictionary, ByVal arguments As String) As String
            Dim fieldExpression As String = expressions(FirstArgument(arguments))
            Dim sb As StringBuilder = New StringBuilder(fieldExpression)
            sb.Append(" in (")
            Dim list() As String = SecondArgument(arguments).Split(New String() {"$or$"}, StringSplitOptions.RemoveEmptyEntries)
            Dim hasNull As Boolean = false
            Dim hasValues As Boolean = false
            For Each v As String in list
    ...
    Public Class NotInFilterFunction
        ...
        Public Overrides Function ExpandWith(ByVal command As DbCommand, ByVal expressions As SelectClauseDictionary, ByVal arguments As String) As String
            Dim list() As String = SecondArgument(arguments).Split(New String() {"$or$"}, StringSplitOptions.RemoveEmptyEntries)
            Dim filter As String = String.Format("not({0})", MyBase.ExpandWith(command, expressions, arguments))
    ...
    Partial Public Class DateRangeFilterFunction
        ...
        Public Overrides Function ExpandWith(ByVal command As DbCommand, ByVal expressions As SelectClauseDictionary, ByVal arguments As String) As String
            ...
            Dim i As Integer = (currentYear - m_StartYear)
    ...
    Public Class ExternalFilterFunction
        ...
        Public Overrides Function ExpandWith(ByVal command As DbCommand, ByVal expressions As SelectClauseDictionary, ByVal arguments As String) As String
            ...
            If (Not (PageRequest.Current) Is Nothing) Then
                Dim parameterName As String = Regex.Match(arguments, "\w+").Value
                For Each v As FieldValue in PageRequest.Current.ExternalFilter
