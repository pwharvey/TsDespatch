...
(function () {
    ...
    _web.DataView = function (element) {
        _app.initializeBase(this, [element]);
    ...
    _app.toHexString = function (v) {
        if (v) {
            var hexValue = ['0x'];
            if (!Array.isArray(v))
                v = v.toString().split($app._simpleListRegex);
            $(v).each(function () {
    ...
    _app.prototype = {
        ...
        multiSelect: function (value) {
            ...
            if (arguments.length) {
                ...
                if (result == null) {
                    selectionMode = _app.touch && that.pageProp('selectionMode');
        ...
        get_filter: function () {
            if (this.get_lookupField() == null && (this.get_pageIndex() == -1 && !this._allFields || this._externalFilter.length > 0 && this._filter.length == 0)) {
                ...
                this.applyExternalFilter(this.get_isModal() || ($app.mobile || this.useCase('$app')) && this._filter);
            }
            ...
            if (!row) {
                if (that.get_isInserting()) {
                    ...
                    if (master) {
                        ...
                        if (extension && extension.commandRow) {
                            masterRow = extension.commandRow();
        ...
        get_selectedRow: function () {
            var extension = this.extension();
            if (extension)
                return $app.touch ? extension.commandRow() : this._selectedRow;
        ...
        batchEdit: function (commandArgument, result) {
            ...
            if (!result)
                ...
                $(result.fields).each(function () {
                    var f = this,
                        tag = f.Tag;
                    if (!f.Hidden && !_field_isReadOnly.call(f) && !f.OnDemand) {
                        editFields.push(f);
                        if (tag)
                        ...
                        if (itemsStyle) {
                            ...
                            if (f.ItemsDataController) {
                                ...
                                $(f.Items).each(function () {
                                    var item = this;
                                    items.values.push({ value: item[0], text: item[1] });
                                });
                            }
                            q.items = items;
                            if (f.AliasName)
        ...
        get_autoHide: function () {
            return !String.isNullOrEmpty(this.get_visibleWhen()) && this._autoHide == Web.AutoHideMode.Nothing ?
        ...
        get_master: function () {
            ...
            return filterSource ? _app.find(filterSource) : null;
        },
        tag: function (tag) {
            this._tag = (this._tag || '') + ',' + tag;
            this._tagList = null;
        },
        untag: function (tag) {
            if (this._tag) {
        ...
        get_isTagged: function (tag) {
            ...
            if (!tagList)
                tagList = that._tagList = that._tag ? that._tag.replace(/,/g, ' ').split(/\s+/) : [];
            return Array.contains(tagList, tag);
        },
        tagged: function (tags) {
            var that = this;
            if (arguments.length == 1)
                return tags.match(/\-$/) ? (that._tag || '').match(new RegExp(tags)) != null : that.get_isTagged(tags);
        ...
        updated: function () {
            ...
            if (!$app.mobile && this.get_modalAnchor() && !this.get_isModal())
        ...
        _updateLayoutContainerVisibility: function (visible) {
            ...
            var activator = !String.isNullOrEmpty(this.dataAttr(c, 'activator')) ? this.dataAttr(c, 'activator').match(/^\s*(\w+)\s*\|\s*(.+?)\s*(\|\s*(.+)\s*)?$/) : null;
        ...
        loadPage: function (showWait) {
            ...
            else if (!Array.contains($app._delayedLoadingViews, this)) {
                Array.add($app._delayedLoadingViews, this);
                $app._startDelayedLoading();
            }
        },
        ...
        groupExpression: function () {
            ...
            if (groupExpression)
                return groupExpression.trim().split($app._simpleListRegex);
            return null;
        },
        groupBy: function () {
            var that = this;
        ...
        showFieldFilter: function (fieldIndex, func, text) {
            ...
            if (func == '$between')
                sb.appendFormat('<div class="Field"><div class="Label"><span class="Function">{0}</span></div><div class="Value"><input type="text" value="{1}"/>{2}</div></div>', Web.DataViewResources.Data.Filters.Labels.And, _app.htmlAttributeEncode(filter[1] ? field.format(this.convertStringToFieldValue(field, filter[1])) : ''), button);
        ...
        showCustomFilter: function (fieldIndex) {
            ...
            var sb = new Sys.StringBuilder();
            sb.appendFormat('<div><span class="Highlight">{0}</span> {1}:</div>', _app.htmlEncode(field.Label), Web.DataViewResources.Data.Filters.Labels.Includes);
        ...
        goToView: function (viewId) {
            ...
            if (v.Type != 'Form') {
                ...
                if (viewChanged && wasForm && $app.mobile)
        ...
        _confirm: function (context, confirmCallback) {
            ...
            if (m && !(__tf != 4)) {
                ...
                var dataView = this.get_isModal() ? null : _app.find(controller, 'Controller');
                if (dataView && !dataView.get_isModal()) {
                    ...
                    if (survey)
                        _app.survey('show', { controller: survey, parent: this._id, confirmContext: context });
                    else
                        dataView = _app.showModal(this, controller, view, commandName, commandArgument, baseUrl, servicePath, [], { confirmContext: context, showSearchBar: this.get_showSearchBar() });
                    result = false;
                }
            }
            else {
        ...
        delegateCommand: function (command, argument) {
            ...
            if (touch && (command.match(/New/) || command.match(/Select|Edit/) && argument && that._viewId != argument) && isForm) {
                parentDataViewId = that._parentDataViewId;
                if (parentDataViewId) {
                    parent = $app.find(parentDataViewId);
                ...
                if (parent && that._controller == parent._controller)
                    touch.pageInfo(that).deleted = true;
                else
                    parent = null;
            }
            return parent;
        ...
        executeRowCommand: function (rowIndex, command, argument, causesValidation, path) {
            ...
            if (primaryDataView)
                ...
                if (rowIndex != null && rowIndex >= 0) {
                    ...
                    that._raiseSelectedDelayed = !(command == 'Select' && String.isNullOrEmpty(argument));
                    if (extension)
                ...
                if (!$app.mobile && that.get_isDataSheet() && command.match(/Select|New|Edit|Duplicate/) && that._rows.length > 0) {
        ...
        executeExport: function (args) {
            ...
            $get('q', f).value = _serializer.serialize(args);
            f.submit();
        },
        ...
        _showModal: function (args) {
            ...
            var dataView = _app.showModal(this, this.get_controller(), args.commandArgument, args.commandName, args.commandArgument, this.get_baseUrl(), this.get_servicePath(),
                this.get_hasParent() || this._lookupInfo ? this._externalFilter : null,
                {
                    'filter': this.get_selectedKeyFilter(), 'ditto': this.get_ditto(), 'lastViewId': this.get_lastViewId(), 'transaction': this.get_transaction(), 'filterSource': this.get_filterSource(), 'filterFields': this.get_filterFields(), 'showSearchBar': this.get_showSearchBar(),
                    'tag': this.get_tag(), 'showActionButtons': this.get_showActionButtons()
                });
            this.set_ditto(null);
            this._savePosition();
        ...
        _actionConfirmed: function (args) {
            var that = this,
                ...
                valid = that._validateFieldValues(actionArgs.Values, args.causesValidation == null || args.causesValidation);
            if (valid) {
                var context = that.get_confirmContext();
                var dataView = that.get_parentDataView();
                if (context && dataView) {
        ...
        _viewDetails: function (fieldName) {
            ...
            if (f) {
                ...
                if (__designer()) {
                    ...
                    var dataView = $app.showModal(this, f.ItemsDataController, "editForm1", "Select", "editForm1", this.get_baseUrl(), this.get_servicePath(), filter, { useCase: 'ObjectRef' });
        ...
        _finishImportUpload: function (frameDocument) {
            ...
            if (errors)
                ...
                for (var i = 0; i < fieldMap.length; i++) {
                    ...
                    sb.appendFormat('<tr><td>{2}</td><td><select id="{0}$ImportField{1}"><option value="">{3}</option>', this.get_id(), i, _app.htmlEncode(mapping[1]), Web.DataViewResources.Data.Import.AutoDetect);
        ...
        updateSummary: function () {
            ...
            if (this._summaryId.length > 0) {
                ...
                if (!$app.mobile && (!empty || clearPermalink) && this.get_filterSource() == null && typeof (Web.Membership) != "undefined") {
            ...
            for (i = 0; i < expressions.length; i++) {
                ...
                if (exp.Type == Web.DynamicExpressionType.ClientScript) {
                    ...
                    if (script) {
                        ...
                        this._javaScriptRowConvert = this.editing();
                        try {
                            //var r = eval(script);
            ...
            switch (field.Type) {
                ...
                case 'DateTime':
                    ...
                    if (typeof v.NewValue == 'string') {
                        ...
                        if (!newValue)
                            newValue = Date.tryParseFuzzyDate(v.NewValue, field.DataFormatString);
                        if (!newValue) {
                            error = resourcesValidator.DateIsExpected;
                        }
                        else
                            v.NewValue = newValue;
                        //if (!error && v.NewValue != null && !skipDateAdjustment)
                        //v.NewValue = new Date(v.NewValue - v.NewValue.getTimezoneOffset() * 60 * 1000);
                    }
            ...
            for (i = 0; i < values.length; i++) {
                ...
                if (field.ReadOnly && field.IsPrimaryKey) {
                    ...
                    if (!field.AllowNulls && (!field.HasDefaultValue || resourcesValidator.EnforceRequiredFieldsWithDefaultValue)) {
                        if (String.isBlank(newValue) && !field.Hidden && !field.isReadOnly())
                            error = resourcesValidator.RequiredField;
                    }
                    // convert blank values to "null"
                    if (!error && String.isBlank(newValue))
                        v.NewValue = null;
            ...
            if (valid && $app.upload())
                $(this._allFields).each(function () {
                    var f = this,
                        test;
                    if (f.OnDemand) {
                        test = $app.upload('validate', { container: findBlobContainer(that, f), dataViewId: that._id, fieldName: f.Name });
            ...
            if (error) {
                ...
                if (message) {
                    ...
                    if (this.get_isForm()) {
                        ...
                        var scrolling = $app.scrolling(); // $common.getScrolling();
        ...
        _internalRender: function () {
            ...
            if (this.get_mode() == Web.DataViewMode.Lookup) {
                ...
                sb.appendFormat('<table cellpadding="0" cellspacing="0" class="DataViewLookup"><tr><td><a href="javascript:" onclick="$find(\'{0}\').showLookup({1});return false" class="Select" id="{0}_Item{1}_ShowLookupLink" title="{3}" tabindex="{7}"{8}>{2}</a><a href="#" class="Clear" onclick="$find(\'{0}\').clearLookupValue({1});return false" id="{0}_Item{1}_ClearLookupLink" title="{5}" tabindex="{7}">&nbsp;</a></td></tr></table><input type="hidden" id="{0}_Item{1}" value="{4}"/><input type="hidden" id="{0}_Text{1}" value="{6}"/>',
                    this.get_id(), field.Index, this.htmlEncode(field, s), String.format(Web.DataViewResources.Lookup.SelectToolTip, field.Label), this.get_lookupValue(), String.format(Web.DataViewResources.Lookup.ClearToolTip, field.Label), _app.htmlAttributeEncode(s), $nextTabIndex(), this.get_enabled() ? '' : ' disabled="true" class="Disabled"');
                    ...
                    if (numCols > 0) {
                        ...
                        for (i = 0; i < categories.length; i++) {
                            ...
                            sb.appendFormat('<tr class="CategoryRow {5}" id="{2}_Category{3}" style="display:{4}"><td valign="top" class="Category {5}"><table class="Category {5}" cellpadding="0" cellspacing="0"><tr><td class="HeaderText">{0}</td></tr><tr><td class="Description" id="{2}$CategoryDescription${3}">{1}</td></tr></table></td><td valign="top" class="Fields {5}">',
                                c.HeaderText, this._formatViewText(Web.DataViewResources.Views.DefaultCategoryDescriptions[description], true, description), this.get_id(), i, !selectedTab || selectedTab == c.Tab ? '' : 'none', c.Id);
                            if (!String.isNullOrEmpty(c.Template))
                                this._renderTemplate(c.Template, sb, row, true, false);
                            else {
                                for (j = 0; j < fields.length; j++) {
        ...
        _renderItem: function (sb, field, row, isSelected, isInlineForm, isFirstRow, headerOnly, trimLongWords, templateMode) {
            ...
            if (isEditing && isSelected && !readOnly) {
                ...
                if (checkBox != null) {
                    ...
                    if (lov.length > 0 || isCheckBoxList || hasItemsStyle && !isLookup && !isAutoComplete) {
                        ...
                        if ($app.supportsPlaceholder && !String.isNullOrEmpty(field.Watermark))
                ...
                if (field._LEV != null) {
                    ...
                    if (lev == null) lev = '';
                    sb.appendFormat('</td><td class="UseLEV"><a href="javascript:" onclick="$find(\'{0}\')._applyLEV({1});return false" tabindex="{2}" title="{3}">&nbsp;</a></td></tr></table>', this.get_id(), field.Index, $nextTabIndex(), _app.htmlAttributeEncode(String.format(dvrd.UseLEV, this._allFields[field.AliasIndex].format(lev))));
                }
                if (this._lastCommandName == 'BatchEdit')
            ...
            if (needObjectRef) {
                ...
                sb.appendFormat('<span class="ObjectRef" title="{0}" onclick="$find(&quot;{1}&quot;).executeCommand({{commandName: &quot;_ViewDetails&quot;, commandArgument: &quot;{2}&quot;}})">&nbsp;</span>',
                    String.format(Web.DataViewResources.Lookup.DetailsToolTip, _app.htmlAttributeEncode(this._allFields[field.AliasIndex].HeaderText)), this.get_id(), field.Name);
        ...
        _isActionAvailable: function (action, rowIndex) {
            var that = this,
                lastCommand = action.WhenLastCommandName ? action.WhenLastCommandName : '';
            if (lastCommand == 'Any')
            ...
            if (available) {
                ...
                else if (action.CommandName == 'BatchEdit')
        ...
        _attachTimeOptions: function (field, element) {
            ...
            if (field.Type == 'TimeSpan') {
                if (!$app._timeSpanOptions)
                    $app._timeSpanOptions = {};
                cache = $app._timeSpanOptions[cacheKey]
                if (!cache) {
                    cache = [];
                    var dt = new Date(),
                    ...
                    while (cache.length < 24 * 4) {
                        Array.add(cache, String.localeFormat(formatString, dt)/*String.format('{0:d2}{1}{2:d2}', dt.getHours(), dtf.TimeSeparator, dt.getMinutes())*/);
                        dt.setMinutes(dt.getMinutes() + 15);
                    }
                    $app._timeSpanOptions[cacheKey] = cache;
                }
            }
            else {
                ...
                if (currentFilter.length > 0 && !this.filterIsExternal()) {
                    for (var i = 0; i < currentFilter.length; i++) {
                        ...
                        if (!filter[2].startsWith('~')) {
                            ...
                            var fd = $app.filterDef(Web.DataViewResources.Data.Filters[field.FilterType].List, field.FilterType == 'Boolean' && m[3].length > 1 ? (m[3] == '%js%true' ? '$true' : '$false') : func);
                            if (fd) {
                                var m2 = v.match(/^(.+?)\$and\$(.+?)$/);
            ...
            for (var i = 0; i < filterDefs.length; i++) {
                ...
                if (fd) {
                    if (fd.List)
                        ...
                        var option = String.format('<option value="{0}"{1}>{2}{3}</option>', v, selected, (fd.Prompt ? '' : Web.DataViewResources.Data.Filters.Labels.Equals + ' '), !fd.Function.match(_app._keepCapitalization) ? fd.Text.toLowerCase() : fd.Text);
                        if (field.SearchOptions) {
        ...
        _renderSearchBarField: function (sb, settings, visibleIndex) {
            ...
            if (isFilter) {
                ...
                fsb.appendFormat('<table class="FilterValues{3}" cellpadding="0" cellspacing="0" onmouseover="$app.highlightFilterValues(this,true,\'Active\')" onmouseout="$app.highlightFilterValues(this,false,\'Active\')"><tr><td class="Values" valign="top"><div><a class="Link" onclick="$find(\'{0}\')._showSearchBarFilter({1},{2});return false;" tabindex="{4}" href="javascript:" onfocus="$app.highlightFilterValues(this,true,\'Focused\')" onblur="$app.highlightFilterValues(this,false,\'Focused\')" title="{5}">', this.get_id(), field.Index, visibleIndex, hasValues ? '' : ' Empty', $nextTabIndex(), Web.DataViewResources.Data.Filters.Labels.FilterToolTip);
                if (hasValues) {
                ...
                fsb.appendFormat('</a></div></td><td class="Button{5}" valign="top"><a href="javascript:" onclick="$find(\'{0}\')._showSearchBarFilter({1},{2});return false" title="{3}" tabindex="{4}" onfocus="$app.highlightFilterValues(this,true,\'Focused\')" onblur="$app.highlightFilterValues(this,false,\'Focused\')" >&nbsp;</a></td></tr></table>', this.get_id(), hasValues ? -1 : field.Index, visibleIndex, hasValues ? Web.DataViewResources.Data.Filters.Labels.Clear : Web.DataViewResources.Data.Filters.Labels.FilterToolTip, $nextTabIndex(), hasValues ? ' Clear' : '');
                button = fsb.toString();
            }
            ...
            if (funcInfo[2] == 'true') {
                ...
                sb.appendFormat('<td class="Control"><input id="{0}$SearchBarValue${1}" type="{6}" class="{2} {7}" value="{3}" tabindex="{4}"/>{5}</td>', this.get_id(), visibleIndex, field.FilterType, _app.htmlAttributeEncode(fieldValue == 'null' ? Web.DataViewResources.HeaderFilter.EmptyValue : fieldValue), $nextTabIndex(), button, isFilter ? 'hidden' : 'text', field.AllowAutoComplete != false ? ' AutoComplete' : '');
            }
            else
                sb.append('<td>&nbsp;</td>');
            ...
            if (field.FilterType != 'Text' && funcInfo[1] == '$between') {
                ...
                sb.appendFormat('<tr><td colspan="2" class="Control AndLabel">{4}</td><td><input id="{0}$SearchBarValue2${1}" type="text" class="{2}" value="{3}" tabindex="{5}"/>{6}</td><td>&nbsp;</td></tr>', this.get_id(), visibleIndex, field.FilterType, _app.htmlAttributeEncode(fieldValue2), Web.DataViewResources.Data.Filters.Labels.And, tabIndex - 1, button);
            }
        },
        ...
        _renderSearchBarControls: function (force) {
            ...
            sb.appendFormat('<div class="SearchButtons"><button onclick="$find(\'{0}\')._performSearch();return false" tabindex="{3}" class="Search">{1}</button><br/><button onclick="$find(\'{0}\')._resetSearchBar();return false" tabindex="{4}" class="Reset">{2}</button></div>',
                this.get_id(), resources.Grid.PerformAdvancedSearch, resources.Data.Filters.Labels.Clear, $nextTabIndex(), $nextTabIndex());
            //if (settings.availableFields.length > 0)
        ...
        _internalRenderActionBar: function (sb) {
            ...
            if (isGrid/*view.Type == 'Grid'*/ && this.get_showQuickFind()) {
                ...
                sb.appendFormat('<td class="QuickFind" title="{2}" id="{0}$QuickFind"><div class="QuickFind"><table cellpadding="0" cellspacing="0"><tr><td><input type="text" id="{0}_QuickFind" value="{1}" class="{3}" tabindex="{4}"/></td><td class="Button"><a href="#" onclick="$find(\'{0}\').quickFind();return false;"><span>&nbsp;</span></a></td></tr></table></div></td>',
                    this.get_id(), _app.htmlAttributeEncode(s), Web.DataViewResources.Grid.QuickFindToolTip, s == Web.DataViewResources.Grid.QuickFindText ? 'Empty' : 'NonEmpty', $nextTabIndex());
                sb.append('<td class="Divider"><div></div></td>');
                            ...
                            if (a._isAvailable)
                                sb.appendFormat(
                                    '<td class="{7}Group FlatGroup{6}{8}{9}" onmouseover="$showHover(this,&quot;{0}${2}$ActionGroup${1}&quot;,&quot;ActionGroup&quot;)" onmouseout="$hideHover(this)" onclick="if(this._skip)this._skip=null;else $find(\'{0}\').executeAction(\'ActionBar\',{1},null,{2})" title="{10}"><span class="Outer">{5}<a href="javascript:" tabindex="{4}" onclick="this.parentNode.parentNode._skip=true;$find(\'{0}\').executeAction(\'ActionBar\',{1},null,{2});return false;" onfocus="$showHover(this,&quot;{0}${2}$ActionGroup${1}&quot;,&quot;ActionGroup&quot;,2)" onblur="$hideHover(this)">{3}</a></span></td>',
                                        ...
                                        !String.isNullOrEmpty(a.CssClass) ? String.format('<span class="FlatGroupIcon {0}">&nbsp;</span>', _app.cssToIcon(a.CssClass)) : '',
        ...
        _renderViewDescription: function (sb) {
            ...
            if (showLetters) {
                ...
                var beginsWith = String.format('{0} {1} ', _app.htmlAttributeEncode(field.Label), Web.DataViewResources.Data.Filters.Text.List[2].Text.toLowerCase());
            ...
            if (!String.isNullOrEmpty(t) || this.get_lookupField() || /*isTree || */lt) {
                ...
                sb.appendFormat('<a href="javascript:" onclick="return false" class="Toggle" title="{1}"><span>&nbsp;</span></a>', this.get_id(), _app.htmlAttributeEncode(Web.DataViewResources.Grid.FlatTreeToggle));
                ...
                _app.htmlAttributeEncode(String.format(Web.DataViewResources.Lookup.SelectToolTip, '"' + levelInfo.text + '"')),
            ...
            for (i = 0; i < currentFilter.length; i++) {
                ...
                while (m && (m[1].startsWith('~') || !(field.Index == field.AliasIndex && field.IsPrimaryKey && field.Hidden))) {
                    if (!first)
                        ...
                        sb.appendFormat('<span class="FilterElement" onclick="$find(\'{0}\').removeFromFilterByIndex({1})" title="{2}">', this.get_id(), isQuickFind ? -1 : field.Index, isQuickFind ? Web.DataViewResources.Data.Filters.Labels.Clear : _app.htmlAttributeEncode(String.format(Web.DataViewResources.HeaderFilter.ClearFilter, field.Label)));
                    ...
                    if (m[1].startsWith('~')) {
                        ...
                        hint = $app.touch && that.viewProp('quickFindHint');
                        if (hint) {
                            hint = hint.split(';');
                            sb.append(' ' + Web.DataViewResources.Mobile.In + ' ');

                            if (hint[0])
                                sb.append(that.get_view().Label);
                            else
                                firstDataView = true;
                            $($app.touch._pages).each(function () {
                                var page = this,
                                    dv = page.dataView,
                                    hintIndex;
                                if (dv) {
                                    if (that._id == dv._filterSource) {
                                        ...
                                        if (hintIndex != -1) {
                                            if (firstDataView)
                                                ...
                                                sb.append(' ' + Web.DataViewResources.Data.Filters.Labels.And + ' ');
                                            else
                                                sb.append(', ');
                        ...
                        var fd = $app.filterDef(Web.DataViewResources.Data.Filters[field.FilterType].List, field.FilterType == 'Boolean' && m[3].length > 1 ? (m[3] == '%js%true' ? '$true' : '$false') : m[1]);
                        if (!fd) {
                            switch (m[1]) {
                                case '=':
                            ...
                            sb.appendFormat(' {0} <b>{1}</b>', Web.DataViewResources.Data.Filters.Labels.Equals, fd.Function.match(_app._keepCapitalization) ? fd.Text : fd.Text.toLowerCase());
                    }
        ...
        _findItemByValue: function (field, value) {
            ...
            return itemCache[value] || [null, this.get_isForm() || _app.touch/* this.get_viewType() == 'Form'*/ ? Web.DataViewResources.Data.NullValueInForms : Web.DataViewResources.Data.NullValue];
        },
        ...
        _forceSync: function () {
            var that = this;
            if (that._skipSync || _app.touch && that.get_isForm())
        ...
        _combinedFilter: function (filter) {
            ...
            if (touch && that.viewProp('useAdvancedSearch')) {
                advancedSearchFilter = that.viewProp('advancedSearchFilter');
        ...
        _createParams: function (filterByPosition) {
            ...
            var confirmContext = that.get_confirmContext(),
                ...
                params = {
                    ...
                    QuickFindHint: $app.touch ? that.viewProp('quickFindHint') : null, RequiresPivot: that._requiresPivot == true, PivotDefinitions: that._pivotDefinitions ? that._pivotDefinitions : null
                };
            if (api) {
        ...
        _loadPage: function () {
            ...
            if (that.get_mode() != Web.DataViewMode.View) {
                that._allFields = [{ Index: 0, Label: '', DataFormatString: '', AliasIndex: 0, ItemsDataController: that.get_controller(), ItemsNewDataView: that.get_newViewId(), ItemsDataView: that.get_viewId(), _dataView: that, Behaviors: [], format: _field_format, isReadOnly: _field_isReadOnly, isNumber: _field_isNumber }];
                ...
                var rules = new Web.BusinessRules(that);
        ...
        _cancelWSRequest: function () {
            ...
            if (webServiceRequest) {
                ...
                if (webServiceRequest.get_executor && !webServiceRequest.completed() && (!Sys.Browser.agent == Sys.Browser.InternetExplorer || Sys.Browser.version > 9))
        ...
        _invoke: function (methodName, params, onSuccess, userContext, onFailure) {
            ...
            function retry(error, response, method) {
                if (error.get_statusCode() == 0) {
                    $app.confirm(Web.DataViewResources.Data.ConnectionLost,
                        function () {
            ...
            if (params.args)
                _app.ensureJSONCompatibility(params.args.Values);
            ...
            if (servicePath.startsWith('http')) {
                ...
                $.ajax({
                    ...
                    type: 'GET',
                    cache: false,
                    data: { method: methodName, args: _serializer.serialize(params) },
                    dataType: 'jsonp',
                    success: function (result) {
        ...
        _supportsCaching: function (viewType) {
            return viewType == 'DataSheet' || viewType == 'Grid' && $app.mobile;
        },
        ...
        _onGetPageComplete: function (result, context) {
            ...
            if (Sys.Services && Sys.Services.AuthenticationService && Sys.Services.AuthenticationService.get_isLoggedIn && Sys.Services.AuthenticationService.get_isLoggedIn() && !result.IsAuthenticated) {
                ...
                location.replace($app.unanchor(location.href));
                return;
            }
            if (!that._fields)
            ...
            if (that._pageIndex < 0 || positionChanged) {
                if (this._pageIndex == -1 || positionChanged) {
                    ...
                    for (i = 0; i < result.Fields.length; i++) {
                        ...
                        if (field.ItemsDataController && field._autoAlias) {
                            var lookupFieldAutoAlias = { Name: field.Name + '_auto_alias_', ReadOnly: true, Type: 'String', Index: result.Fields.length, Label: field.Label, AllowNulls: true, Items: [], htmlEncode: true };
                            lookupFieldAutoAlias.AliasIndex = lookupFieldAutoAlias.Index;
                            field.AliasIndex = lookupFieldAutoAlias.Index;
                            result.Fields.push(lookupFieldAutoAlias);
                    ...
                    if ($app.newValues) {
                        if (!this._ditto)
                            this._ditto = [];
                        this._ditto = this._ditto.concat($app.newValues);
                        $app.newValues = null;
                    }
                    ...
                    for (i = 0; i < result.Fields.length; i++) {
                        ...
                        if (field.TextMode == 2 && String.isNullOrEmpty(field.Editor) && !touch) {
                    ...
                    for (i = 0; i < this._actionGroups.length; i++) {
                        ...
                        for (j = 0; j < ag.Actions.length; j++) {
                            ...
                            if (agt && String.isNullOrEmpty(action.HeaderText)) {
            ...
            var rules = new Web.BusinessRules(this);
            rules.after({ commandName: result.NewRow ? 'New' : 'Select', commandArgument: result.View, view: result.View });
            rules.dispose();
        },
        ...
        _autoSelect: function (rowIndex) {
            ...
            if (!this._tryFocusDataSheet || this._forceFocusDataSheet) {
                ...
                if (this.get_isDataSheet() && !this.get_lookupField() & (!this.get_filterSource() || this._forceFocusDataSheet) && this._rows.length > 0 && !_app.mobile)
        ...
        findAction: function (path) {
            ...
            $(this._actionGroups).each(function () {
                var ag = this;
                if (ag.Id == groupId)
                    $(ag.Actions).each(function () {
                        var a = this;
                        if (a.Id == actionId) {
                            result = a;
                            return false;
                ...
                if (exitCommands) {
                    for (var i = 0; i < exitCommands.length; i++) {
                        if (commandName == exitCommands[i]) {
                            ...
                            if ($app.mobile)
                                $app.mobile.endModalState(parentDataView, this);
                            else
                                this.dispose();
                            return true;
                        }
        ...
        _createArgsForListOfValues: function (distinctFieldName) {
            var lc = this.get_lookupContext(),
                ...
                request = {
                    ...
                    QuickFindHint: $app.touch ? this.viewProp('quickFindHint') : null
                };
        ...
        get_selectedValues: function () {
            ...
            return selection.length == 0 ? [] : (!dataView.multiSelect() ? [selection] : selection.split(';'));
        },
        ...
        _populateDynamicLookups: function (result) {
            ...
            if ($app.mobile)
                ...
                $app.input.execute({ dataView: this, values: result.Values, populateDynamicLookups: false });
            else {
        ...
        _notifyMaster: function () {
            if (this.get_hasParent()) {
                ...
                if (m) m._updateDynamicValues('controller:' + this.get_controller());
            }
        },
        ...
        _onExecuteComplete: function (result, context) {
            var that = this,
                extension = that.extension(),
                mobile = $app.mobile,
                lastCommandName = that._lastArgs.CommandName,
            ...
            if (pendingUploads && pendingUploads.length > 0 && noErrors && lastCommandName.match(/^(Insert|Update)$/)/* && result.RowsAffected == 1*/) {
                ...
                if (key == null)
                    key = that._findKeyValue(that._lastArgs.Values);
                if (upload.files) {
                    ...
                    $app.upload('execute', {
                        container: findBlobContainer(that, blobField), // $(_app.touch ? _app.touch.page(that._id) : that._container).find('.drop-box-' + blobField.Index),
                        files: upload.files,
            ...
            if (noErrors) {
                ...
                var lastCommandArgument = that._lastArgs.CommandArgument;
                var rules = new Web.BusinessRules(that);
                ...
                if (that._continueAfterScript) {
                    if (result.NavigateUrl) {
                        ...
                        that.navigate(result.NavigateUrl, existingRow ? that._lastArgs.Values : resultValues);
                    }
                    else if (that._closeInstantDetails()) { }
                    else if (that.endModalState(lastCommandName)) { }
                    else if (that.get_backOnCancel() || !String.isNullOrEmpty(that._replaceTriggerViewId)) that.goBack(true);
                    else {
                        ...
                        for (i = 0; i < actions.length; i++) {
                            ...
                            if (a.WhenLastCommandName == lastCommandName && (!a.WhenLastCommandArgument || a.WhenLastCommandArgument == lastCommandArgument) && that._isActionMatched(a)) {
                                ...
                                //        dataView.executeCommand({ commandName: a.CommandName, commandArgument: a.CommandArgument, path: a.Path, causesValidation: a.CausesValidation, values: lastCommandName.match(/Insert|Custom/) ? lastArgsValues : null });
                                //        dataView._pendingSelectedEvent = a.CommandName.match(/^(Edit|Select)/) != null;
                                //        dataView._notifyMaster();
                                //    }
                                //}
                                //if (mobile && dataView._parentDataViewId) {
                                //    dataView = $app.find(that._parentDataViewId);
                                //    mobile.endModalState(dataView, that, executeNextAction);
                                //}
                                //else
                                //    executeNextAction();
                                commandName = a.CommandName;
                        ...
                        if (!stopFlow) {
                            ...
                            if (that.get_isModal() && (!that.get_isForm(that._lastViewId) || that._closeViewDetails))
        ...
        _displayActionErrors: function (result) {
            ...
            for (i = 0; i < result.Errors.length; i++)
                sb.append(_app.formatMessage('Attention', result.Errors[i]));
            _app.showMessage(sb.toString());
            sb.clear();
        },
        _busy: function (isBusy) {
            ...
            var that = this,
                ...
                atWindowTop = !mobile && (that.get_isModal() || that.get_lookupField() != null || !this._controller) || mobile != null || this.get_useCase() == '$app',
            ...
            if (mobile) {
                clearTimeout($app._busyIndicatorTimeout);
                busyIndicator = $app._busyIndicator;
                if (!busyIndicator) {
                ...
                if (isBusy) {
                    if (!_app._busyCount) {
                        ...
                        busyIndicator.removeClass('dataview-busy-indicator-done').css({ top: $container.outerHeight() - busyIndicator.outerHeight() - 1, 'animation-duration': Math.max(1000, $window.width()) / 1000 * 3000 + 'ms' });
            ...
            if (isBusy && !interval) {
                ...
                var $busy = that._$busy = $(String.format('<div class="dataview-busy-indicator{0}"></div>', atWindowTop ? ' modal' : ''))
                    .css({ position: mobile ? 'fixed' : 'absolute', width: w, 'z-index': atWindowTop ? (mobile ? 5000 : 3000000) : 0 }),
                    x = -w,
                ...
                this._busyInterval = setInterval(function () {
                    ...
                    x += Math.ceil(w / 30);
                    if (doShow) {
                        if (!atWindowTop || !$app._busyIndicator) {
                            $app._busyIndicator = true;
                            $busy.show();
                        }
                        else {
                            isHidden = true;
                    ...
                    else if (isHidden && !$app._busyIndicator) {
                        isHidden = false;
                        $app._busyIndicator = true;
                        $busy.show();
                    }
        ...
        _filterSourceSelected: function (sender, args, keepContext) {
            ...
            this.applyExternalFilter($app.mobile);
            ...
            for (var i = 0; i < allFields.length; i++) {
                ...
                if (contextFields && !map[i] && f.ItemsStyle) {
                    ...
                    while (m = iterator.exec(contextFields)) {
                        if (m[3] == fieldName) {
                            map[i] = true;
                            values.push({ 'name': f.Name, 'value': null });
                            var aliasField = allFields[f.AliasIndex];
                            if (aliasField != f)
                                values.push({ 'name': aliasField.Name, 'value': null });
                            var copy = f.Copy,
                                m2;
                            if (copy)
            ...
            if (!String.isNullOrEmpty(field.Copy) && (field.ItemsStyle == 'RadioButtonList' || field.ItemsStyle == 'ListBox' || field.ItemsStyle == 'DropDownList')) {
                ...
                if (selectedItem) {
                    ...
                    if ($app.touch)
                        ...
                        $app.input.execute({ dataView: this, values: values });
                    else
                        this.refresh(true, values);
                    return true;
                }
    ...
    _app.loadDesigner = function () {
        if (!_app.Designer) {
            // load css
            var cssLink = 'http://localhost:' + __designerPort + '/touch/codeontime.designer.css';
            ...
            if (typeof (__cotdesigner) != 'undefined')
                ...
                $.ajax('http://localhost:' + __designerPort + '/touch/codeontime.designer.css');
                $.getScript('http://localhost:' + __designerPort + '/Scripts/codeontime.designer.js', function () {
    ...
    function validateDate(date) {
        return date && date.getTimezoneOffset && !(isNaN(+date) || date < new Date(1753, 0) || date > new Date(9999, 0));
    }

    _app.ensureJSONCompatibility = function (values) {
        $(values).each(function () {
            var v = this;
            if (v.OldValue && v.OldValue.getTimezoneOffset)
                v.OldValue = _app.stringifyDate(v.OldValue);
            if (v.NewValue && v.NewValue.getTimezoneOffset)
                v.NewValue = _app.stringifyDate(v.NewValue);
        });

    }


    _app.stringifyDate = function (d, field) {
        if (!validateDate(d))
            return d;
    ...
    _app.execute = function (args) {
        var placeholder = $('<div/>').appendTo($('body')).hide(),
            ...
            dataView = $create(Web.DataView, {
                ...
                servicePath: args.url || __servicePath, baseUrl: __baseUrl, useCase: '$app', tags: args.tags,
        ...
        dataView._onExecuteComplete = function (result, context) {
            done();
            if (args.success) {
                var r = result ?
                {
                ...
                $(args.values).each(function () {
                    var v = this;
                    obj[v.name] = typeof v.newValue == 'undefined' ? (typeof v.oldValue == 'undefined' ? v.value : v.oldValue) : v.newValue;
                });
    ...
    _app.find = function (id, propertyName) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (propertyName) {
                ...
                if (c._id == id || _app.isInstanceOfType(c) && c._id.endsWith(cid)) return c;
        }
        if (arguments.length == 1)
            return $app.find(id, 'Controller');
        return null;
    }
    ...
    _app.showModal = function (parent/*anchor*/, controller, view, startCommandName, startCommandArgument, baseUrl, servicePath, filter, properties) {
        ...
        if (properties) {
            if (!properties.useCase) {
                properties.useCase = $app._defaultUseCase;
                $app._defaultUseCase = null;
            }
        ...
        if (parentIsDataView) {
            ...
            if (mobile && mobile.pageInfo(parent._id).deleted) {
        ...
        if (survey) {
            if (survey.description)
                if (_app.touch)
                    survey.text = [survey.text, survey.description];
                else {
            ...
            survey.compiled = function (result) {
                if (mobile) {
                    ...
                    mobile.modalDataView(params.id, true);
                }
                else
    ...
    _app.filterDef = function (filterDefs, func) {
        ...
        for (var i = 0; i < filterDefs.length; i++) {
            ...
            if (fd) {
                if (fd.List) {
                    var result = $app.filterDef(fd.List, func);
    ...
    _app.callWithFeedback = function (link, callback) {
        if ($app.mobile)
            $app.mobile.callWithFeedback(link, callback);
        else
            callback();
    }
    ...
    _app.login = function (username, password, createPersistentCookie, success, error) {
        $app._invoke('Login', { username: username, password: password, createPersistentCookie: createPersistentCookie }, function (result) {
            if (result) {
                if (success)
                    success();
            }
    ...
    _app.roles = function (callback) {
        $app._invoke('Roles', {}, function (result) {
            if (callback)
                callback(result);
        });
    }
    ...
    _app.configureFramework = function (framework, pageContent, callback) {
        $($app.contentFrameworks[framework]).each(function () {
            var fw = this;
    ...
    _window._body_resizeLayoutContainers = function () {
        ...
        var bounds = $app.bounds(pc);
        var padding = $app.paddingBox(pc);
        //var padding2 = $common.getPaddingBox(pc);
        var border = $app.borderBox(pc);
        //var border2 = $common.getBorderBox(pc);
        var margin = $app.marginBox(pc); // $common.getMarginBox(pc);
        ...
        while (rowIndex >= layoutContainers[0].rowIndex) {
            ...
            for (var i = 0; i < layoutContainers.length; i++) {
                ...
                if (lc.rowIndex == rowIndex && !String.isNullOrEmpty(lc.width)) {
                    var div = $get(lc.id);
                    if (div) {
                        var divPadding = $app.paddingBox(div); // $common.getPaddingBox(div);
                        var divBorder = $app.borderBox(div); // $common.getBorderBox(div);
                        var divMargin = $app.marginBox(div); // $common.getMarginBox(div);
            ...
            if (usedSpace < bounds.width) {
                for (i = 0; i < layoutContainers.length; i++) {
                    ...
                    if (lc.rowIndex == rowIndex && String.isNullOrEmpty(lc.width)) {
                        div = $get(lc.id);
                        if (div) {
                            divPadding = $app.paddingBox(div); // $common.getPaddingBox(div);
                            divBorder = $app.borderBox(div); // $common.getBorderBox(div);
                            divMargin = $app.marginBox(div); // $common.getMarginBox(div);
    ...
    _window._body_scroll = function () {
        var sideBar = $getSideBar();
        if (!sideBar) return;
        var scrolling = $app.scrolling(); // { x: $(window).scrollLeft(), y: $(window).scrollTop() };// $common.getScrolling();
        var clientBounds = $common.getClientBounds();
    ...
    _window._body_createPageContext = function (persist) {
        ...
        var ctx = { 'height': b.height - pb.vertical - bb.vertical, 'scrolling': $app.scrolling()/* $common.getScrolling()*/ };
    ...
    _window._body_performResize = function () {
        ...
        //_body_scroll();
        if (typeof (__cothost) == 'undefined') {
            var bounds = $app.bounds(pc); // $common.getBounds(pc);
            var padding = $app.paddingBox(pc); // $common.getPaddingBox(pc);
            var border = $app.borderBox(pc); // $common.getBorderBox(pc);
    ...
    _app._activate = function (source, elementId, type) {
        ...
        if (type == 'SiteAction' && elem.childNodes[0].className != 'CloseSiteAction') {
            ...
            closeLink.innerHTML = String.format('<a href="javascript:" onclick="$app._activate(null,\'{0}\',\'SiteAction\')">{1}</a>', elementId, Web.DataViewResources.ModalPopup.Close);
        ...
        if (pc) {
            body.find('div[data-app-role="page"]').each(function () {
                ...
                if (framework)
                    if (!hasContent && (!hash || hash[1] == pageId)) {
                        ...
                        body.addClass('app-sidebar-undocked').addClass('app-theme-' + __settings.theme.toLowerCase());
                        hasContent = true;
                        content = page.find('> div[data-role="content"]').addClass('app-bootstrap app-content-desktop');
                        $app.configureFramework(framework, content);
            ...
            $(pc).find('div[data-role="placeholder"]').each(function () {
                var placeholder = $(this),
                    factory = $app._contentFactories[placeholder.attr('data-placeholder')];
                if (factory)
                    factory(placeholder);
            });
            //userPages.remove();
            ...
            for (i = 0; i < divs.length; i++) {
                ...
                if (!String.isNullOrEmpty(width) || !String.isNullOrEmpty(flow)) {
                    ...
                    for (var j = 0; j < childDivs.length; j++) {
                        ...
                        if (!String.isNullOrEmpty(activator) && !String.isBlank(childDiv.innerHTML)) {
    ...
    _app.get_commandLine = function () {
        ...
        if (!commandLine) {
            ...
            _app._commandLine = $app.unanchor(commandLine);
        }
        return commandLine;
    }
    ...
    */
    ...
    _window._field_prepareDataFormatString = function (dataView, field) {
    ...
    _window._field_format = function (v) {
        try {
            if (v != null && this.Type == 'TimeSpan' && this.DataFormatString && this.FormatOnClient && typeof v == 'string')
                v = Date.tryParseFuzzyTime(v, false);
            return v == null ? 'null' : this.FormatOnClient && !String.isNullOrEmpty(this.DataFormatString) ? String.localeFormat(this.DataFormatString, v) : v.toString();
        }
        catch (e) { throw new Error(String.format('\nField: {0}\nData Format String: {1}\n{2}', this.Name, this.DataFormatString, e.message)) }
    }

    _window._field_isReadOnly = function () {
        return this.TextMode == 4 || this.ReadOnly;
    }

    _window._field_isNumber = function () {
        return Array.indexOf(['SByte', 'Byte', 'Int16', 'Int32', 'UInt32', 'Int64', 'Single', 'Double', 'Decimal', 'Currency'], this.Type)
    }

    _window._field_htmlEncode = function () {
        return this.HtmlEncode && this.TextMode != 2;
    }

    _window._field_trim = function (v) {
        var that = this,
            dvrd = Web.DataViewResources.Data;
        if (that.Type == 'String' && v != null && v.length > dvrd.MaxReadOnlyStringLen && !(that.TextMode == 3 || that.TextMode == 2))
            v = v.substring(0, dvrd.MaxReadOnlyStringLen) + '...';
    ...
    _window._field_text = function (v, trim) {
        var that = this,
            s, valueList,
            nullValue = Web.DataViewResources.Data.NullValueInForms,
        ...
        if (m) {
            ...
            if (!String.isNullOrEmpty(hh)) {
                ...
                if (!String.isNullOrEmpty(ampm))
                    ...
                    if (autoAdjustHours != false && !String.isNullOrEmpty(dtf.PMDesignator) && dtf.ShortTimePattern.indexOf('tt') > 0 && new Date().getHours() >= 12)
                        hh += 12;
                d.setHours(hh);
            }
    ...
    function compileBusinessRule(script) {
        ...
        return compiledScript.join('');
    }

    _web.BusinessRules = function (dataView) {
        this._dataView = dataView;
        this.result = new Web.BusinessRules.Result(this);
    }

    _web.BusinessRules.prototype = {
        canceled: function () {
            ...
            if (wdvg && wdvg.acquired && !(__tf != 4) && (commandName.match(/New|Edit/) || commandName.match(/Update|Insert/) && phase == 'Before')) {
                ...
                if (newValues)
                    if ($app.mobile)
                        ...
                        $app.input.execute({ dataView: dataView, values: newValues });
                    else
            ...
            if (!that._rules) {
                ...
                for (i = 0; i < expressions.length; i++) {
                    ...
                    if (exp.Type == Web.DynamicExpressionType.ClientScript && exp.Scope == Web.DynamicExpressionScope.Rule) {
                        m = exp.Result.match(/^<id>(.+?)<\/id><commandName>(.+?)<\/commandName><commandArgument>(.*?)<\/commandArgument><view>(.*?)<\/view><phase>(.*?)<\/phase><script>([\s\S]+?)<\/script>$/);
                        if (m) {
                            var ruleName = String.format('Web.BusinessRules.{0}_Rule_{1}', dataView._controller, m[1] ? m[1] : ruleIndex),
                                ruleFunc = eval(String.format('typeof {0} !="undefined"?{0}:null', ruleName));
                            if (!ruleFunc)
                                try {
            ...
            if (file) {
                ...
                if (fileResult.Values.length)
                    if (_app.mobile)
                        ...
                        $app.input.execute({ dataView: dataView, values: fileResult.Values });
                    else
    ...
    $app.dataUrlToBlob = function (url) {
        var byteString,
            mimeString,
            byteArray, i;
    ...
    function findBlobContainer(dataView, blobField) {
        return $(_app.touch ? _app.touch.page(dataView._id) : dataView._container).find('.drop-box-' + blobField.Index);
    }

    $app.upload = function (method, options) {
        if (!arguments.length || !uploadSupport.formData)
            return uploadSupport.formData;

        var container = $(options.container),
            clickEvent = _app.touch ? 'vclick' : 'click';
        ...
        function validate() {
            var dv = _app.find(options.dataViewId),
                blobField = dv.findField(options.fieldName),
                signature = container.data('signature'),
                row = _app.touch ? dv.extension().commandRow() : dv.get_currentRow(),
            ...
            if (container.is('.app-signature')) {
                ...
                filesSelected(dv, options.fieldName, signed ? $app.dataUrlToBlob(signature.toDataURL('image/png', 1)) : null);
            }
            else
        ...
        function configureEmpty() {
            ...
            if (uploadSupport.dragAndDrop && (!touch || touch.desktop()))
                container.text(multiple ? resourcesFiles.DropMany : resourcesFiles.Drop);
            else
                container.text(touch ? (multiple ? resourcesFiles.TapMany : resourcesFiles.Tap) : (multiple ? resourcesFiles.ClickMany : resourcesFiles.Click));
            container.attr('title', touch ? (multiple ? resourcesFiles.TapMany : resourcesFiles.Tap) : (multiple ? resourcesFiles.ClickMany : resourcesFiles.Click));
        }
        ...
        function initialize() {
            if (!container.is('.app-drop-box')) {
                ...
                var dataView = $app.find(options.dataViewId),
                    ...
                    $('<a class="app-clear ui-btn ui-btn-inline ui-corner-all"/>').insertAfter(container).text(resourcesFiles.Clear).attr('title', resourcesFiles.Clear)
                        .on(clickEvent, function (e) {
                            if (container.data('signature').isEmpty()) {
                                if (_app.touch)
                                    _app.touch.activeLink();
                                return;
                            }
                            container.addClass('app-dragging');
                            $app.confirm(resourcesFiles.ClearConfirm, function () {
                                container.removeClass('app-dragging').focus();
                                resizeSignature();
                                if (options.dataViewId) {
                                    var dataView = _app.find(options.dataViewId);
                    ...
                    container.on('dragenter', function () {
                        ...
                        if (!container.is('.app-uploading'))
                            if ($(e.target).closest('.app-clear').length) {
                                container.addClass('app-dragging')
                                $app.confirm(container.find('div').map(function () { return $(this).text() }).get().join(', ') + '\n\n' + resourcesFiles.ClearConfirm, function () {
                                    container.removeClass('app-dragging').focus();
                                    configureEmpty();
                                    container.next().val('');
                                    if (options.dataViewId) {
        ...
        function captureFiles(files) {
            ...
            var imageFiles = 0,
                dv = _app.find(options.dataViewId),
                modalHeight = dv && !_app.touch && dv.get_isModal() ? $(dv._container).height() : 0;
            ...
            $(files).each(function (index) {
                var file = this,
                    ...
                    size = file.size > 1024 ? String.format('{0:N0} ', file.size / 1024) + resourcesFiles.KB : file.size + ' ' + resourcesFiles.Bytes,
            ...
            $('<a class="app-clear ui-btn ui-btn-icon-notext ui-icon-delete ui-corner-all"/>').appendTo(container).text(resourcesFiles.Clear).attr('title', resourcesFiles.Clear);
            if (dv)
                filesSelected(dv, options.fieldName, files);
        }

        function execute() {
            uploadFiles(options.files);
        }
        ...
        function uploadFiles(files) {
            ...
            container.addClass('app-uploading');
            progress.val(0).insertBefore(container.contents().first());
            xhr.open('POST', options.url);
            xhr.onload = function () {
                container.removeClass('app-uploading');
                if (options.success)
                    options.success();
            };
    ...
    _app.survey = function (method, options) {
        ...
        function showCompiled(survey) {
            ...
            var dataView = $app.find(options.parent);
            survey.controller = controller;
            survey.baseUrl = dataView.get_baseUrl();
            survey.servicePath = dataView.get_servicePath();
            survey.confirmContext = options.confirmContext;
            survey.showSearchBar = dataView.get_showSearchBar();
            survey.parent = options.parent;
            _app.showModal(dataView, options.controller, 'form1', 'New', '', survey.baseUrl, survey.servicePath, [],
                { confirmContext: options.confirmContext, showSearchBar: survey.showSearchBar, survey: survey });
        }

        function createRule(list, func, commandName, commandArgument, phase) {
            var s = typeof func === 'string' ? ('function(){$(document).trigger($.Event("' + func + '", {rules:this}))}') : func.toString(),
                m = s.match(/^function\s*\(\)\s*\{([\s\S]+?)\}\s*$/);
            if (!commandArgument)
                commandArgument = '';
            list.push({
                "Scope": 6, "Target": null, "Type": 1, "Test": null,
                "Result": "\u003cid\u003er" + list.length + "\u003c/id\u003e\u003ccommandName\u003e" + commandName + "\u003c/commandName\u003e\u003ccommandArgument\u003e" + commandArgument + "\u003c/commandArgument\u003e\u003cview\u003eform1\u003c/view\u003e\u003cphase\u003e" + phase + "\u003c/phase\u003e\u003cscript\u003e" + m[1] + "\u003c/script\u003e",
                "ViewId": 'form1'
            });
        }
        ...
        function iterate(topics, parent, topicCallback, questionCallback) {
            $(topics).each(function () {
                ...
                $(t.questions).each(function () {
                    var q = this;
                    if (questionCallback)
                        questionCallback(q, t, parent)
                });
                if (t.topics)
                    iterate(t.topics, t, topicCallback, questionCallback);
            });
        }
            ...
            $(list).each(function () {
                ...
                if (contextFields) {
                    ...
                    while (m = _app._fieldMapRegex.exec(contextFields)) {
                        field = dataView.findField(m[2]);
                        if (field)
                            filter.push(m[1] + ':=' + dataView.convertFieldValueToString(field, row[field.Index]));
                    }
                    if (filter.length)
        ...
        function compile() {
            var requiresItems = [],
                result = {
                    ...
                    Views: [{ Id: 'form1', Label: options.text, Type: 'Form', HeaderText: options.description }],
                    Expressions: [
                    ],
            ...
            $(options.topics).each(function (index) {
                var topic = this,
                ...
                $(topic.questions).each(function () {
                    var fd = this,
                        fdType = fd.type || 'String',
                    ...
                    if (fdContext)
                        f.ContextFields = typeof fdContext === 'string' ? fdContext : fdContext.join(',');
                    ...
                    if (items) {
                        f.ItemsStyle = items.style || (items.values ? 'DropDownList' : 'Lookup');
                        f.Items = [];
                        if (items.autoCompleteAnywhere)
                            f.SearchOptions = '$autocompleteanywhere';
                        if (items.values) {
                            $(items.values).each(function () {
                                var item = this;
                                f.Items.push([item.value, item.text]);
                                if (items.other)
                                    $(item.other).each(function () {
                                        f.Items.push(this);
                                    });
                            });
                        }
                            ...
                            if (!f.ItemsStyle.match(/AutoComplete|Lookup/) && !f.ContextFields)
            ...
            if (options.init)
                createRule(result.Expressions, options.init, 'New', 'form1', 'After');
            if (options.submit)
                createRule(result.Expressions, options.submit, 'Confirm', null, 'Before');
            if (options.cancel)
                createRule(result.Expressions, options.cancel, 'Cancel', null, 'Before');
            var compileCallback = options.compiled;
            if (compileCallback) {
                if (requiresItems.length)
                    populateItems(requiresItems, result.Fields, function () {
        ...
        if (method == 'show')
            if (options.dynamic)
                ...
                if (survey)
                    ...
                    $.ajax({
                        url: dataView.get_baseUrl() + '/scripts/surveys/' + controller + '.js',
                        dataType: 'text',
                        cache: false,
                        success: function (result) {
                            busy(false);
            ...
            $(dataView._allFields).each(function () {
                var f = this;
                if (f.ItemsAreDynamic && f.ContextFields)
                    fieldWithContext.push(f);
            });
            if (fieldWithContext.length)
                populateItems(fieldWithContext, dataView._allFields, options.callback);
        }
        else
