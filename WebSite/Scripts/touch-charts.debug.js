...
(function () {
    ...
    var $window = $(window),
        ...
        updateHeaderTimeout,
        drawTimeInterval,
        scrollingAnimationCallback,
        cancelScrollingTimeout,
        animationSpeed = 500,
        timeoutSpeed = 100,
        refreshSpeed = 250,
        weekTimeWidth = 66,
                ...
                if (button.is('.app-calendar-today,.app-calendar-next,.app-calendar-prev')) {
                    ...
                    if (tabs.css('visibility') == 'hidden') {
                        ...
                        touch.callWithFeedback(button, function () {
                            touch.listPopup({
                                anchor: button,
                                title: button.attr('data-cal-value')
                            });
                        });
                    }
                }
        ...
        move: function (drag) {
            ...
            if (that._startMarginLeft != null) {
                ...
                that.scrollingAnimationCallback = function () {
                    skipClick = true;
                    that._handler._scroll(that._calView, that._calHeader, -newMarginLeft, that._calFooter);
    ...
    $app.dragMan['calendar-bar-day'] = calendarDragManager;
    ...
    var eventDragManager = {
        ...
        start: function (drag) {
            ...
            if (context) {
                ...
                this._oldHeight = appEvent.outerHeight();
                ...
                if (calendar.mode.match(/day|week/)) {
                    ...
                    this._placeholder = $('<span style="display:none"></span>').insertAfter(appEvent);
                }
            }
                ...
                case 'week':
                    ...
                    if (dropTarget.length) {
                        ...
                        var date = dropTarget.is('.current-time-line') ? dropTarget.data('date') : getDateOfBlock(dropTarget),
                            ...
                            scrollablePos = scrollable.position();
                        ...
                        if (!dragScrollHorizTimeout) {
                            var distLeft = drag.x - scrollablePos.left - 60,
                                    ...
                                    doLoad = distLeft < 20 || distRight < 20;
        ...
        cancel: function (drag) {
            ...
            if (Math.abs(drag.x - this._startX) < 5)
                target.trigger('vclick');

            if (this._hasMoved)
                this._element.removeClass('app-event-preview');

            if (calendar.mode.match(/day|week/)) {
                var origColumn = calendar.getBlockByDate(calendar.getVisibleView(), this._context[1]);
                if (origColumn.length)
                    this._element.appendTo(origColumn.find('ul'));
                else
                    this._element.remove();
            }
            else { // month
                if (this._hasMoved)
                    this._element.insertAfter(this._placeholder);
                this._placeholder.remove();
            }

            $('.app-calendar').removeClass('app-has-time-prompt');
            setTimeout(drawTime, timeoutSpeed * 2);
            this._element.css({ height: this._oldHeight, top: this._oldTop });
    ...
    Calendar.prototype = {
        ...
        show: function (options) {
            ...
            if (calendar.length > 0) {
                ...
                // first time setup 
                calendar = $('<div class="app-calendar" data-draggable="calendar"></div>').appendTo(options.container);

                if (!isEcho) {
                    // create a tab bar in the fixed header
                    header = $('<div class="app-bar-calendar"></div>').appendTo($app.touch.bar('create', { type: 'header', page: options.container }));
                    ...
                    // create a footer bar
                    footer = $('<div class="app-bar-calendar-footer"> </div>').appendTo($app.touch.bar('create', { type: 'footer', page: options.container }));
                    var outer = $('<div class="app-scroll-outer app-bar-system"></div>').insertBefore(footer.contents().first()),
                    ...
                    outer
                        .on('scroll', function (event) {
                            ...
                            if (mode.match(/day|week/)) {
                                var sl = outer.scrollLeft(),
                                    ...
                                    wasNull = scrollingAnimationCallback == null;

                                scrollingAnimationCallback = function () {
                        ...
                        .on('scrollstop', function () {
                            clearTimeout(cancelScrollingTimeout);
                            cancelScrollingTimeout = setTimeout(function () {
                                scrollingAnimationCallback = null;
                            }, 400);
                        });
        ...
        getFirstVisibleBlock: function (scrollable) {
            if (!scrollable)
                scrollable = this.scrollable();
            var headerHeight = touch.stickyHeaderBar().outerHeight() + scrollable.offset().top, // TODO
                blocks = $(this.getBlocks(scrollable)),
        ...
        updateHeader: function (header) {
            var that = this;
            if (updateHeaderTimeout)
                return;

            updateHeaderTimeout = setTimeout(function () {
                ...
                while (badge[0].clientWidth + 5 > space) {
    ...
    Day.prototype = {
        ...
        draw: function (container, startDate) {
            ...
            var dayHeaders = $('<div class="app-day-header" data-draggable="calendar-bar-day"></div>').appendTo(header),
    ...
    Week.prototype = {
        draw: function (container, startDate) {
            ...
            var dayHeaders = $('<div class="app-week-header" data-draggable="calendar-bar-week"></div>').appendTo(header),
...
date = getDateOfBlock(visible);
    ...
    function scaleTransition(from, to, hiddenCallback, afterCallback) {
        ...
        setTimeout(function () {
            from.one('transitionend', function () {
                from.hide();
                if (hiddenCallback)
                    hiddenCallback();
    ...
    var CalendarInput = touch.CalendarInput = (function (method, options) {
        var outerContainer = options.container,
            input = options.input,
            dataView = options.dataView = options.dataView || (input && $app.find(input.closest('[data-input-container]').attr('data-input-container'))) || touch.dataView(),
        ...
        if (input && input.length) {
            if (!input.parent().length && !touchPointer) {
                alert('TODO: input is detached!');
                return;
            }
            var wrapper = input.closest('.app-wrapper');

            // find global form control
            if (!outerContainer)
                outerContainer = wrapper.find('.app-calendar-plugin-container');
            if (!outerContainer.length)
                outerContainer = $('<div class="app-calendar-plugin-container app-data-input-helper"></div>').hide().appendTo(wrapper);
            $('<div class="app-calendar-cover" style="display:none"></div>').appendTo(wrapper);
            options.container = outerContainer;
            dataInput = input.closest('[data-input]');
            options.field = field = $app.input.elementToField(dataInput);
            fieldName = field.Name;
            outerContainer.data('data-input', dataInput);
                    ...
                    if (field) {
                        ...
                        if (options.endField) {
                            ...
                            if (!field.TimeFmtStr || (options.oldTimeMode == 'Minute' && options.target.closest('.app-time-selector').length)) {
                                setTimeout(function () {
                                    input.blur();
                                    $app.input.execute({ dataView: dataView, values: [{ name: options.endField, value: options.date }] });
                                    setTimeout(function () {
            ...
            // add handlers
            if (!input.attr('data-datepicker-attached')) {
                input
                    .attr('data-datepicker-attached', 'true')
                    .focus(function () {
                        var top,
                            left,
                            containerHeight = outerContainer.height(),
                            containerWidth = outerContainer.width(),
                            wrapperWidth = wrapper.width(),
                            wrapperHeight = wrapper.height(),
                            scrollTop = wrapper.scrollTop(),
                            showToolbar = touchPointer,
                            cover = $('.app-calendar-cover');
                        ...
                        if ($window.width() < 640) {
                            ...
                            top = pos.top - wrapperPos.top + scrollTop + input.outerHeight();
                            ...
                            // check right
                            if (left + containerWidth > wrapperWidth)
                                left = wrapperWidth - containerWidth - 10; // shadow
                        }
                        outerContainer.css({ top: top, left: left });
                        outerContainer.data('select-date', options.date);
                        if (showToolbar) {
                            outerContainer.addClass('app-calendar-touch');
                            cover.show();
                        }
                        else {
                            outerContainer.removeClass('app-calendar-touch');
                            cover.hide();
                        }

                        if (touchPointer)
                            outerContainer.stop(true).fadeIn('fast');
                        else
                            outerContainer.show();

                        if (showToolbar || touchPointer)
                            input.blur();
                    })
                    .on('input', function () {
                        var dv = $app.find(dataViewId),
                            d = getInputDate(dv, input.val());
                        if (validateDate(d))
                    ...
                    .on('remove.input.app', function () {
                        if (!outerContainer.is('.app-calendar-touch')) {
                            CalendarInput('hide', { container: outerContainer });
                            outerContainer.removeData('select-date');
                        }
                    });
        ...
        function getInputDate(dv, dateStr) {
            var dObj = { NewValue: dateStr };
            dv._validateFieldValueFormat(field, dObj, true);
            return dObj.NewValue;
        }

        function setInput(dataView, field, input, date) {
            var valid = validateDate(date),
                value = valid ? field.format(date) : '',
                placeholder;
            if (!valid && !field.AllowNulls) {
                date = new Date();
                value = field.format(date);
            }
            if (touchPointer || !input || input.parent().length == 0)
                $app.input.execute({ dataView: dataView, values: [{ name: field.Name, value: date }] });
            else {
                $app.input.value(value);
                //input.closest('[data-input]').find('.app-data-input-placeholder').css('display', value ? 'none' : '');
                //input.val(value);
            }
        }
        function hide() {
        ...
        options.excludeKeyFieldsFromFilter = true;
        options.queryData = function (field, fieldName) {
        ...
        switch (method) {
            case 'setInput':
                setInput(options.dataView, $app.input.elementToField(dataInput), dataInput.find('input'), options.date);
                break;
            case 'clear':
                setInput(options.dataView, $app.input.elementToField(dataInput), dataInput.find('input'));
                break;
            case 'hide':
                hide();
                break;
    ...
    var CalendarControl = (function (method, options) {
        ...
        var container = outerContainer.find('.app-calendar-plugin'),
            ...
            dataView = options.dataView || touch.dataView(),
            ...
            $(data).each(function () {
                ...
                if (val && val != 0) {
                    ...
                    var keyValues = /(\d+), (\d+)/.exec(key),
                        monthN = parseInt(keyValues[1], 10) - 1,
                        day = keyValues[2],
        ...
        function refresh() {
            ...
            switch (mode) {
                case '':
                    ...
                    header.html(String.format(miniCalendarHeaderFormat,
                        dtf.MonthNames[drawDate.getMonth()], drawDate.getFullYear(),
                        dtf.MonthNames[drawDate.getMonth()], drawDate.getFullYear()));
                    ...
                    if (linkWidth > header.innerWidth() - 60)
                        header.html(String.format(miniCalendarHeaderFormat,
                            dtf.MonthNames[tempDate.getMonth()], drawDate.getFullYear(),
                            dtf.AbbreviatedMonthNames[tempDate.getMonth()], drawDate.getFullYear()));

                    if (drawDate.getFullYear() != firstDate.getFullYear() || drawDate.getMonth() != firstDate.getMonth()) {
                        reloadData = true;

                        tempDate.setMonth(tempDate.getMonth() - showMonths);
            ...
            function asyncRequestContextHasChanged(field, activeFieldName) {
                if (!outerContainer.is(':visible'))
                    return true;
                if ($mobile.activePage.attr('id') != activePageId)
                    return true;
        ...
        function attach() {
            var row = dataView.extension().commandRow(),
                createContainer = !container.length;

            // create mini calendar
            if (createContainer) {

                container = $('<div class="app-calendar-plugin"></div>').prependTo(outerContainer);
                ...
                monthContainer = $('<div class="app-month-container" data-draggable="calendar-mini"></div>').appendTo(container);
                var monthContainerInner = $('<div class="app-scroll-inner"></div>').appendTo(monthContainer),
                    beforeMonths = $('<div class="app-scroll-column"></div>').appendTo(monthContainerInner),
                    duringMonths = $('<div class="app-scroll-column"></div>').appendTo(monthContainerInner),
                    afterMonths = $('<div class="app-scroll-column"></div>').appendTo(monthContainerInner);

                selectorContainer = $('<div class="app-date-selector-container" data-draggable="calendar-mini" style="display:none"></div').appendTo(container);
                var selectorInner = $('<div class="app-scroll-inner"><div class="app-scroll-column"></div><div class="app-scroll-column"></div><div class="app-scroll-column"></div></div>').appendTo(selectorContainer);
                ...
                if (options.renderTime) {
                    ...
                    if (!options.hideActionBar)
                        actionBar = $('<div class="app-calendar-action-bar">'
                            ...
                            + '<a class="ui-btn ui-corner-all ui-mini ui-btn-inline app-calendar-btn-clear"/>'
                            + '</div>').appendTo(outerContainer);
                }
        ...
        switch (method) {
            ...
            case 'clearCache':
                ...
                break;
            default:
                $app.alert('Attempted to run CalendarControl("' + method + '").');
        }
        return result;
    });
    ...
    CalendarPlugin.prototype = {
        attach: function (sidebar) {
                ...
                calendar = CalendarControl('attach', {
                    ...
                    onrefresh: function (options) {
                        ...
                        var fieldSelector = container.find('.app-calendar-plugin-fieldselector'),
                            ...
                            filterIndicator = container.find('.glyphicon-filter');


                        var filter = dataView.combinedFilter(),
                            scrollable = $.mobile.activePage.find('.app-wrapper'),
                            calendarView = scrollable.find('.app-calendar'),
                            calendarActive = calendarView.length && calendarView.is(':visible'),
                            calendarConfig = dataView.calendar;
            ...
            if (!calendarExisted) {
                var fieldSelector = $('<a class="ui-btn ui-btn-icon-none app-has-droparrow app-calendar-plugin-fieldselector"><span class="glyphicon glyphicon-filter" style="opacity:0"></span><span class="text">' + activeField.Label + '</span></a>').appendTo(calendar),
    ...
    $(document)
        .on('vclick', '.app-calendar-color-legend', function (event) {
            var target = $(event.target),
                sidebar = touch.sidebar();
            // handle color filter
        ...
        .on('vclick mousedown', '.app-calendar-plugin, .app-calendar-plugin-container', function (event) {
            ...
            if (target.is('td')) {
                ...
                if (mode == '') {
                    if (isLeft)
                        date.setMonth(date.getMonth() - showMonths);
                    else
                        date.setMonth(date.getMonth() + showMonths);
                ...
                var is24hour = dtf.AMDesignator == '',
                    ...
                    lastTouch = $app.touch.lastTouch() || { x: event.pageX, y: event.pageY },
        ...
        .on('touchstart mousedown pointerdown', '.app-calendar-cover', function (event) {
            event.preventDefault();
            CalendarInput('hide', { container: $('.app-calendar-plugin-container') });
            return false;
        })

        .on('beforefocus.input.app', '[data-input][data-type="date"],[data-input][data-type="datetime"]', function (event) {
            if (!CalendarInput('attach', { input: event.inputElement }))
                event.preventDefault();
        })

        .on('cancel.input.app', '[data-input][data-type="date"],[data-input][data-type="datetime"]', function (event) {
        ...
        .on('help.input.app', '[data-input][data-type="date"],[data-input][data-type="datetime"]', function (event) {
            // handler "Ctrl+Space" or "Ctrl+Down"  pressed in the input - show the visible calendar
            $('.app-data-input-helper').show();
            event.preventDefault();
        });
...
(function () {
    var isTouchPointer = !$app.touch.desktop(),
        _window = window,
        $window = $(_window),
        $body = $(window.body),
        iOS = $body.is('.app-ios');

    $app.input.methods.richtext = {
        extend: function (options) {
            ...
            v = row[field.Index];
            if (v == null)
                t = resources.NullValueInForms;
            else
                t = field.format(v);
            inner.html(t);
            if (field.Rows)
                inner.height(field.Rows + 'em');
            if (options.editing) {
        ...
        focus: function (target) {
            ...
            target.attr('contentEditable', 'true').addClass('app-rich-text-editor');
